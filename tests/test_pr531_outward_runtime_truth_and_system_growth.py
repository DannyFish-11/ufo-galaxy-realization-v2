...
class TestDeploymentBaseline:
    ...
    def test_core_runtime_baseline_met(self):
        ...
        report = check_runtime_baseline(DeploymentEnvironment.CORE_RUNTIME)
        assert report.baseline_met, f"CORE_RUNTIME baseline_met=False; " f"failed checks: {report.summary_notes}"
    ...
    def test_standard_dev_baseline_met(self):
        ...
        assert report.baseline_met, f"STANDARD_DEV baseline_met=False; " f"failed checks: {report.summary_notes}"
    ...
    def test_report_passed_count_matches_requirements(self):
        ...
        report = check_runtime_baseline(DeploymentEnvironment.CORE_RUNTIME)
        actual_passed = sum(1 for r in report.requirements if r.status == BaselineCheckStatus.PASSED)
    ...
    def test_report_failed_count_matches_requirements(self):
        ...
        report = check_runtime_baseline(DeploymentEnvironment.CORE_RUNTIME)
        actual_failed = sum(1 for r in report.requirements if r.status == BaselineCheckStatus.FAILED)
...
class TestValidateRuntimeSections:
    ...
    def test_validate_runtime_main_includes_new_checks(self):
        ...
        for expected in [
            ...
            assert expected in call_names, f"{expected} not found in validate_runtime.py main() call list"
