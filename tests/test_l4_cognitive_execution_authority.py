...
def test_36_execute_raises_when_context_lacks_authority_sentinel():
    ...
    with pytest.raises(ValueError, match="context_assembly authority"):
        asyncio.new_event_loop().run_until_complete(auth.execute(req))
...
def test_51_execute_returns_cognitive_execution_result():
    ...
    result = asyncio.new_event_loop().run_until_complete(auth.execute(req))
...
def test_85_retry_path_same_context_and_supply_yields_same_provider():
    ...
    res1 = asyncio.new_event_loop().run_until_complete(auth.execute(req1))
    res2 = asyncio.new_event_loop().run_until_complete(auth.execute(req2))
