...
╚═══════════════════════════════════════════════════════════╝
        ...
        for i, test in enumerate(tests, 1):
            ...
            try:
                ...
                print(f"❌ EXCEPTION: {e}")
                self.results.append(
                    MissionResult(name=test.__name__, passed=False, duration_ms=0, details={}, error=str(e))
                )
    ...
    async def test_2_simple_query_routing(self) -> MissionResult:
        ...
        response = await self.client.post(f"{MODEL_ROUTER_URL}/route", json={"prompt": prompt, "prefer_local": False})
    ...
    async def test_5_lock_management(self) -> MissionResult:
        ...
        response = await self.client.post(
            ...
            json={"node_id": "test_node", "resource_id": "test_resource", "token": token},
        )
