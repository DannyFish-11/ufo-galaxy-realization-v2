...
class TestModuleImportAndSentinels:
    ...
    def test_a4_authority_sentinel_mentions_pr11(self):
        ...
        assert "PR11" in sentinel or "PR-11" in sentinel, "Authority sentinel should reference PR-11"
...
class TestConvergencePolicySentinels:
    ...
    def test_d6_boundary_clarity_policy_references_all_five_boundaries(self):
        ...
        for name in expected_names:
            assert (
                name in BOUNDARY_CLARITY_PRESERVED_POLICY
...
class TestBoundarySentinelCompletenessCheck:
    ...
    def test_g2_all_five_boundary_sentinels_checked(self):
        ...
        assert len(info_findings) >= 5, (
            "Expected at least 5 info findings (one per boundary sentinel) " f"but got {len(info_findings)}"
        )
    ...
    def test_g3_all_three_policy_sentinels_checked(self):
        ...
        assert len(info_findings) >= 8, (
            "Expected at least 8 info findings (5 boundary + 3 policy) " f"but got {len(info_findings)}"
        )
...
class TestTerminalGuardIntegrityCheck:
    ...
    def test_h2_terminal_guards_module_found(self):
        ...
        assert any(
            "terminal_architecture_audit_guards" in m for m in info_msgs
    ...
    def test_h3_five_guard_policy_sentinels_verified(self):
        ...
        info_findings = [f for f in findings if f.severity == "info"]
        assert len(info_findings) >= 7, f"Expected at least 7 info findings, got {len(info_findings)}"
    ...
    def test_h4_run_terminal_audit_guards_passes(self):
        ...
        errors = [f for f in findings if f.severity == "error"]
        assert not errors, (
            "Terminal audit guards should pass on a clean system — " f"errors found: {[f.message for f in errors]}"
        )
...
class TestParticipantLayerConsistencyCheck:
    ...
    def test_i4_generic_above_android_policy_verified(self):
        ...
        info_msgs = [f.message for f in findings if f.severity == "info"]
        assert any(
            "PARTICIPANT_GENERIC_LAYER_IS_ABOVE_ANDROID_CONCRETE_POLICY" in m for m in info_msgs
    ...
    def test_i5_android_concrete_modules_present(self):
        ...
        android_info = [f for f in findings if f.severity == "info" and "android" in f.message.lower()]
        assert android_info, "Expected at least one info finding confirming an Android concrete module is present"
...
class TestCanonicalLayerModelReachableCheck:
    ...
    def test_j3_all_five_layer_keys_present(self):
        ...
        info_msgs = [f.message for f in findings if f.severity == "info"]
        assert any(
            "five canonical layer keys" in m for m in info_msgs
    ...
    def test_j4_all_four_not_sentinels_present(self):
        ...
        for sentinel in not_sentinels:
            assert any(sentinel in m for m in info_msgs), f"Expected info finding confirming {sentinel!r} is present"
    ...
    def test_j5_layer_model_invariants_pass(self):
        ...
        info_msgs = [f.message for f in findings if f.severity == "info"]
        assert any(
            "run_layer_model_invariants" in m and "passed" in m for m in info_msgs
...
class TestRunFinalConvergenceChecks:
    def test_k1_runs_all_four_checks(self):
        ...
        assert len(report.checks_run) == 4, f"Expected 4 checks_run, got {len(report.checks_run)}: {report.checks_run}"
    ...
    def test_k3_to_dict_contains_all_expected_keys(self):
        ...
        assert expected_keys.issubset(set(d.keys())), f"Missing keys in to_dict(): {expected_keys - set(d.keys())}"
    ...
    def test_k4_generated_at_is_recent_positive_timestamp(self):
        ...
        assert before <= report.generated_at <= after, (
            f"generated_at {report.generated_at} not in expected range " f"[{before}, {after}]"
        )
    ...
    def test_k5_checks_run_names_match_expected(self):
        ...
        assert expected_checks == set(report.checks_run), (
            f"checks_run mismatch. Expected: {sorted(expected_checks)}, " f"Got: {sorted(report.checks_run)}"
        )
...
class TestArchitectureStoryCoherence:
    ...
    def test_l5_no_residual_split_brain_in_key_sentinels(self):
        ...
        assert (
            "NOT" in ORCHESTRATION_SESSION_SCOPE_BOUNDARY_SENTINEL
