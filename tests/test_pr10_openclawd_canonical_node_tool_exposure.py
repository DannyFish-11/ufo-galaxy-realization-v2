    ...
    def test_open_not_called_for_node_registry_json(self) -> None:
        ...
        with patch.dict(os.environ, {"OPENCLAWD_LEGACY_NODE_SCAN_COMPAT_ENABLED": "false"}):
                ...
                patch.dict(
                    "sys.modules",
                    {
                        "core.unified.capability_resolver": MagicMock(
                            get_capability_resolver=MagicMock(
                                return_value=MagicMock(collect_tool_schemas=MagicMock(return_value=[]))
                            )
                        ),
                        "core.nodes.node_fabric_registry": MagicMock(
                            get_node_fabric_registry=MagicMock(
                                return_value=MagicMock(sync_capabilities_to_registry=MagicMock(return_value=0))
                            )
                        ),
                        "core.unified.capability_contract": MagicMock(),
                        "core.mcp_gateway": MagicMock(
                            get_mcp_gateway=MagicMock(
                                return_value=MagicMock(list_generated_tools=MagicMock(return_value=[]))
                            )
                        ),
                    },
                ),
            ...
            assert registry_opens == [], (
                f"node_registry.json was opened unexpectedly when compat is " f"disabled: {registry_opens}"
            )
