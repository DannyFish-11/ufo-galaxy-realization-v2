...
def log_skip(name: str, reason: str):
    ...
    test_results["details"].append({"name": name, "passed": None, "message": f"Skipped: {reason}"})
...
def test_protocol_messages():
    ...
    try:
        ...
        # 测试设备注册确认
        msg = MessageBuilder.device_register_ack(device_id="test_device", success=True, session_id="session_123")
        ...
        # 测试错误消息
        msg = MessageBuilder.error(device_id="test_device", error_code="E001", error_message="Test error")
...
def test_android_bridge():
    ...
    try:
        ...
        device = AndroidDevice(
            device_id="test_android_001", device_type=DeviceType.ANDROID_PHONE, name="Test Phone", model="Pixel 6"
        )
...
def test_node_directories():
    ...
    log_test(
        "节点 main.py 存在", nodes_with_main == len(node_dirs), f"{nodes_with_main}/{len(node_dirs)} 个节点有 main.py"
    )
