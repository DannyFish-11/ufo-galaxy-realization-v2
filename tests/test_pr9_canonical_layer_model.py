...
class TestCanonicalLayerEnum:
    ...
    def test_canonical_layer_keys_matches_enum(self):
        from core.canonical_layer_model import CANONICAL_LAYER_KEYS, CanonicalLayer
...
class TestCanonicalLayerRegistry:
    def test_all_layers_in_registry(self):
        from core.canonical_layer_model import CANONICAL_LAYER_REGISTRY, CanonicalLayer

        for layer in CanonicalLayer:
            assert (
                layer in CANONICAL_LAYER_REGISTRY
    ...
    def test_layer_declaration_has_required_fields(self):
        ...
        for layer in CanonicalLayer:
            ...
            for attr in required:
                assert hasattr(decl, attr), f"LayerDeclaration for {layer.value} missing attribute '{attr}'"
    ...
    def test_all_canonical_modules_non_empty(self):
        ...
        for layer in CanonicalLayer:
            decl = CANONICAL_LAYER_REGISTRY[layer]
            assert len(decl.canonical_modules) > 0, f"Layer '{layer.value}' must have at least one canonical_module"
...
class TestNotPolicySentinels:
    ...
    def test_boundary_layers_have_not_policies(self):
        ...
        for layer in boundary_layers:
            decl = CANONICAL_LAYER_REGISTRY[layer]
            assert len(decl.not_policies) > 0, f"Layer '{layer.value}' should have at least one NOT-policy"
...
class TestV4Invariant:
    def test_v4_per_request_gate_true_is_error(self):
        ...
        assert any("V4_IS_NOT_PER_REQUEST_GATE" in e.detail.get("violation", "") for e in errors)
...
class TestV6Invariant:
    def test_v6_per_request_gate_true_is_error(self):
        ...
        assert any("V6_IS_NOT_PER_REQUEST_GATE" in e.detail.get("violation", "") for e in errors)
...
class TestL1L2L3Invariant:
    def test_router_layer_shadow_stack_true_is_error(self):
        ...
        assert any("L1_L2_L3_BELONGS_TO_ROUTER_LAYER_NOT_SHADOW_STACK" in e.detail.get("violation", "") for e in errors)
...
class TestCompletionTruthInvariant:
    def test_completion_truth_optional_signaling_true_is_error(self):
        ...
        assert any("COMPLETION_TRUTH_IS_ENFORCED_NOT_OPTIONAL" in e.detail.get("violation", "") for e in errors)
...
class TestArchitectureModulesAgreeOnLayerModel:
    ...
    def test_v4_not_per_request_gate_policy_present_in_spine(self):
        ...
        assert (
            "NOT" in spine.V4_IS_NOT_PER_REQUEST_GATE_POLICY.upper() or "not" in spine.V4_IS_NOT_PER_REQUEST_GATE_POLICY
        )
    ...
    def test_no_key_declaration_misclassifies_v4_as_universal_gate(self):
        ...
        assert (
            "NOT" in policy or "not" in policy.lower()
