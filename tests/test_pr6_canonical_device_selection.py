...
class TestDeviceParticipationStatus:
    ...
    def test_to_dict_serialisable(self):
        ...
        s = DeviceParticipationStatus(runtime_present=True, routable=True, cross_device_eligible=True)
...
class TestCanonicalDeviceSelectionEntry:
    ...
    def test_to_dict_has_device_and_participation(self):
        ...
        dev = _make_device("d2")
        entry = CanonicalDeviceSelectionEntry(device=dev, participation=DeviceParticipationStatus(registered=True))
        d = entry.to_dict()
        assert "device" in d
...
class TestAssessDeviceParticipation:
    ...
    def test_online_connected_device_fully_eligible(self):
        ...
        dev = _make_device(online=True, status="online", conn_state="connected", runtime_enabled=True)
        status = assess_device_participation(dev)
        assert status.registered is True
        assert status.runtime_present is True
    ...
    def test_router_liveness_affirms_routability(self):
        ...
        dev = _make_device(device_id="d1", online=True, status="online", conn_state="disconnected")
    ...
    def test_router_liveness_denies_routability(self):
        ...
        dev = _make_device(device_id="d1", online=True, status="online", conn_state="connected", runtime_enabled=True)
    ...
    def test_router_liveness_does_not_change_registered(self):
        ...
        dev = _make_device(device_id="d1", online=True, status="online", conn_state="connected")
        status = assess_device_participation(dev, router_liveness={"d1": False})
...
class TestAssessParticipationEnrichment:
    ...
    def test_no_autonomy_still_orch_eligible_when_runtime_present(self):
        ...
        dev = _make_device(online=True, status="online", conn_state="connected", runtime_enabled=False)
        status = assess_device_participation(dev)
...
class TestSelectOrchestrationCandidates:
    def test_only_orchestration_eligible_returned(self):
        ...
        orch = _make_device("o1", online=True, status="online", conn_state="connected", runtime_enabled=True)
    ...
    def test_capability_filter_excludes_missing(self):
        ...
        dev = _make_device("c1", online=True, status="online", conn_state="connected", capabilities=["screen"])
        result = select_orchestration_candidates([dev], required_capabilities=["screen", "camera"])
        assert result == []
    ...
    def test_capability_filter_includes_matching(self):
        ...
        dev = _make_device(
            "c2", online=True, status="online", conn_state="connected", capabilities=["screen", "camera", "microphone"]
        )
        result = select_orchestration_candidates([dev], required_capabilities=["screen", "camera"])
        assert len(result) == 1
...
class TestDeviceScoreInputFromCanonical:
    ...
    def test_latency_and_load(self):
        ...
        entry = self._make_entry()
        result = device_score_input_from_canonical(entry, ping_latency_ms=42.5, load_pct=30.0)
...
class TestCrossDeviceCoordinatorCanonicalHelpers:
    ...
    def test_get_canonical_devices_by_type_filters(self):
        ...
        with patch("galaxy_gateway.cross_device_coordinator.device_router") as mock_router:
            ...
            result = coord._get_canonical_devices_by_type(DeviceType.ANDROID, cross_device_eligible_only=False)
...
class TestSwarmCoordinatorCanonicalBridge:
    ...
    def test_device_candidates_capability_filter(self):
        ...
        result = coord.device_candidates_from_canonical([dev_has, dev_missing], required_capabilities=["camera"])
    ...
    def test_device_candidates_latency_map(self):
        ...
        result = coord.device_candidates_from_canonical([dev], latency_map={"s5": 99.0})
        assert len(result) == 1
    ...
    def test_device_candidates_router_liveness_enrichment(self):
        ...
        result = coord.device_candidates_from_canonical([dev], router_liveness={"s6": True})
        assert len(result) == 1
...
class TestEligibilitySemantics:
    ...
    def test_orchestration_requires_cross_device_eligible(self):
        ...
        dev = _make_device("d2", online=True, status="online", conn_state="disconnected", runtime_enabled=True)
        status = assess_device_participation(dev)
        assert status.cross_device_eligible is False
        assert status.orchestration_eligible is False
