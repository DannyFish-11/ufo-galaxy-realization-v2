...
class TestRecordEntryModeDiff:
    ...
    def test_returns_record_when_enabled(self, monkeypatch):
        monkeypatch.setenv("GALAXY_DECISION_DIFF_TELEMETRY", "1")
        from core.decision_diff_telemetry import DecisionDiffRecord, record_entry_mode_diff
    ...
    def test_record_stored_in_ring_buffer(self, monkeypatch):
        monkeypatch.setenv("GALAXY_DECISION_DIFF_TELEMETRY", "1")
        from core.decision_diff_telemetry import get_diff_store, record_entry_mode_diff
...
class TestRecordCandidateDiff:
    ...
    def test_returns_record_when_enabled(self, monkeypatch):
        monkeypatch.setenv("GALAXY_DECISION_DIFF_TELEMETRY", "1")
        from core.decision_diff_telemetry import DecisionDiffRecord, record_candidate_diff
    ...
    def test_record_stored_in_ring_buffer(self, monkeypatch):
        monkeypatch.setenv("GALAXY_DECISION_DIFF_TELEMETRY", "1")
        from core.decision_diff_telemetry import get_diff_store, record_candidate_diff
...
class TestNoFinalDecisionSideEffects:
    ...
    def _resolve(self, env_enabled: bool, use_readiness: bool, monkeypatch) -> str:
        monkeypatch.setenv("GALAXY_DECISION_DIFF_TELEMETRY", "1" if env_enabled else "0")
        monkeypatch.setenv("GALAXY_ENTRYMODE_USE_READINESS", "1" if use_readiness else "0")
