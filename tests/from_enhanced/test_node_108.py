...
class TestNode108MetaCognition:
    ...
    def test_track_thought(self):
        ...
        payload = {
            ...
            "context": {"task": "system_enhancement", "user_query": "系统能否反思自己？"},
        }

        response = requests.post(f"{NODE_URL}/api/v1/track_thought", json=payload, timeout=TIMEOUT)
    ...
    def test_track_thought_invalid_type(self):
        """测试追踪思维 - 无效类型"""
        payload = {"thought_type": "invalid_type", "content": "测试内容", "context": {}}
    ...
    def test_track_decision(self):
        ...
        response = requests.post(f"{NODE_URL}/api/v1/track_decision", json=payload, timeout=TIMEOUT)
    ...
    def test_evaluate_decision(self):
        ...
        payload = {
            ...
            "outcome": {"success_score": 0.9, "actual_result": "成功开发并部署", "time_taken": 7200},
        }

        response = requests.post(f"{NODE_URL}/api/v1/evaluate_decision", json=payload, timeout=TIMEOUT)
    ...
    def test_evaluate_decision_not_found(self):
        """测试评估决策 - 决策不存在"""
        payload = {"decision_id": "nonexistent_decision", "outcome": {"success_score": 0.9}}
    ...
    def test_evaluate_decision_missing_success_score(self):
        ...
        payload = {"decision_id": decision_id, "outcome": {"actual_result": "成功"}}
    ...
    def test_reflect(self):
        ...
        # 反思最近的活动
        payload = {"time_window": 60}  # 最近60秒

        response = requests.post(f"{NODE_URL}/api/v1/reflect", json=payload, timeout=TIMEOUT)
    ...
    def test_optimize_strategy(self):
        ...
        payload = {
            ...
            "current_strategy": {"approach": "逐个工具开发专用节点", "priority": "OpenCode 优先", "timeline": "2周"},
        }

        response = requests.post(f"{NODE_URL}/api/v1/optimize_strategy", json=payload, timeout=TIMEOUT)
    ...
    def test_get_cognitive_state(self):
        """测试获取认知状态"""
        response = requests.get(f"{NODE_URL}/api/v1/cognitive_state", timeout=TIMEOUT)
    ...
    def test_get_thought_history(self):
        """测试获取思维历史"""
        response = requests.get(f"{NODE_URL}/api/v1/thought_history?limit=10", timeout=TIMEOUT)
    ...
    def test_get_decision_history(self):
        """测试获取决策历史"""
        response = requests.get(f"{NODE_URL}/api/v1/decision_history?limit=10", timeout=TIMEOUT)
    ...
    def test_cognitive_bias_detection(self):
        """测试认知偏差检测"""
        # 测试确认偏差
        payload = {"thought_type": "analysis", "content": "这个证据证实了我的假设，验证了我的想法", "context": {}}
        ...
        # 测试过度自信
        payload = {"thought_type": "decision", "content": "我肯定这个方案一定会成功，绝对没问题", "context": {}}
    ...
    def test_complete_workflow(self):
        ...
        eval_response = requests.post(
            ...
            json={"decision_id": decision_id, "outcome": {"success_score": 0.85}},
            timeout=TIMEOUT,
        )
        assert eval_response.status_code == 200

        # 4. 反思
        reflect_response = requests.post(f"{NODE_URL}/api/v1/reflect", json={"time_window": 60}, timeout=TIMEOUT)
        assert reflect_response.status_code == 200

        # 5. 优化策略
        optimize_response = requests.post(
            f"{NODE_URL}/api/v1/optimize_strategy",
            json={"task_description": "工具集成", "current_strategy": {"approach": "通用包装器"}},
            timeout=TIMEOUT,
        )
