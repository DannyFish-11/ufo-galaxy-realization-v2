...
# ---------------------------------------------------------------------------
# 2. capability_report -> accepted
# ---------------------------------------------------------------------------

@pytest.mark.asyncio
async def test_capability_report_accepted(bridge, mock_ws):
    """Sending capability_report must be accepted by the bridge."""
...
# ---------------------------------------------------------------------------
# 4. task_result -> processed (no response, but no exception)
# ---------------------------------------------------------------------------
...
async def test_task_result_processed(bridge, mock_ws):
    ...
    # task_result handler returns None (fire-and-forget)
    response = await bridge.handle_message(mock_ws, message)
    # A None response is acceptable; what matters is no exception was raised
