...
class TestResolveEntryModeDefault:
    def test_returns_local_by_default(self):
        ...
        from core.unified.entrypoint_router import reset_entrypoint_router, resolve_entry_mode
...
class TestOpenClawdEntryMode:
    ...
    async def test_process_with_explicit_local(self):
        ...
        assert meta.get("entry_mode") == "local", f"Expected entry_mode='local' in metadata, got {meta}"
    ...
    async def test_process_defaults_to_local_when_not_provided(self):
        ...
        meta = result.get("metadata", {})
        assert meta.get("entry_mode") == "local", f"Expected entry_mode='local' (default) in metadata, got {meta}"
    ...
    async def test_process_cross_device_echoed(self):
        ...
        result = await clawd.process(message="hello", entry_mode="cross_device", session_id="test-sess-3")
        meta = result.get("metadata", {})
        assert meta.get("entry_mode") == "cross_device", f"Expected entry_mode='cross_device' in metadata, got {meta}"
...
class TestDesktopPresenceRuntimeEntryMode:
    ...
    async def test_handle_request_passes_entry_mode_to_openclawd(self):
        ...
        assert (
            captured.get("entry_mode") == "cross_device"
