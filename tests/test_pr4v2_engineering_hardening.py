...
class TestStrictPreflightMode:
    ...
    def test_system_orchestrator_accepts_strict_preflight_kwarg(self):
        ...
        assert (
            "strict_preflight" in sig.parameters
    ...
    def test_strict_preflight_false_by_default(self):
        ...
        try:
            ...
            assert orch.strict_preflight is False, "strict_preflight should be False by default when env var is not set"
        finally:
    ...
    def test_strict_preflight_enabled_via_env_var(self):
        ...
        try:
            ...
            orch = SystemOrchestrator()
            assert orch.strict_preflight is True, "strict_preflight should be True when GALAXY_STRICT_PREFLIGHT=1"
        finally:
    ...
    def test_main_py_references_strict_preflight(self):
        ...
        with open(main_path) as f:
            content = f.read()
        assert (
            "GALAXY_STRICT_PREFLIGHT" in content
    ...
    def test_strict_mode_exception_returns_false(self):
        ...
        try:
            ...
            assert result is False, "In strict mode, an unhandled preflight exception must return False"
        finally:
            os.environ.pop("GALAXY_STRICT_PREFLIGHT", None)
            if old_val is not None:
                os.environ["GALAXY_STRICT_PREFLIGHT"] = old_val
    ...
    def test_non_strict_mode_exception_returns_true(self):
        ...
        try:
            ...
            with mock.patch(
                ...
                result = main_module._run_orchestrator_preflight()
            assert result is True, "In non-strict mode, an unhandled preflight exception must still return True"
        finally:
            os.environ.pop("GALAXY_STRICT_PREFLIGHT", None)
            if old_val is not None:
                os.environ["GALAXY_STRICT_PREFLIGHT"] = old_val
...
class TestAsyncIoBoundaryHardening:
    ...
    def test_llm_manager_does_not_use_deprecated_get_event_loop_in_reload(self):
        ...
        deprecated_calls = [
            node for node in ast.walk(reload_node) if isinstance(node, ast.Attribute) and node.attr == "get_event_loop"
        ]
