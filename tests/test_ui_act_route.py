...
class TestUIAct:
    def test_structural_hit_dispatches_via_invoke_node(self, monkeypatch):
        ...
        out = asyncio.run(
            ui_act.ui_act(
                UIActRequest(instruction="点「发送」", ui_graph=_wechat_graph(), node_id="Node_36_UIAWindows")
            )
        )
    ...
    def test_set_text_carries_text(self, monkeypatch):
        ...
        out = asyncio.run(
            ui_act.ui_act(
                UIActRequest(
                    instruction="在「输入消息」输入「你好」", ui_graph=_wechat_graph(), node_id="Node_36_UIAWindows"
                )
            )
        )
    ...
    def test_absent_target_needs_model_no_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr("core.node_invocation.invoke_node", _fake_invoke)
        out = asyncio.run(ui_act.ui_act(UIActRequest(instruction="点「悬浮球」", ui_graph=_wechat_graph())))
    ...
    def test_dry_run_plans_without_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr("core.node_invocation.invoke_node", _fake_invoke)
        out = asyncio.run(
            ui_act.ui_act(UIActRequest(instruction="点「发送」", ui_graph=_wechat_graph(), execute=False))
        )
...
class TestNodeIdSanitization:
    def test_malicious_node_id_falls_back_to_safe_default(self, monkeypatch):
        ...
        monkeypatch.setattr("core.node_invocation.invoke_node", _fake_invoke)
        out = asyncio.run(
            ui_act.ui_act(UIActRequest(instruction="点「发送」", ui_graph=_wechat_graph(), node_id="../../etc/passwd"))
        )
        assert out["dispatched"]
