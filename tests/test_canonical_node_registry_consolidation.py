...
class TestNodeRegistryCompatFacade:
    ...
    def test_get_node_registry_returns_node_registry_instance(self):
        from core.node_registry import NodeRegistry, get_node_registry
...
class TestNodesStatusEndpointLogic:
    ...
    def test_legacy_only_nodes_appear_with_source_legacy(self):
        ...
        from core.node_registry import NodeMetadata
        from core.node_registry import NodeStatus as LegacyStatus
...
class TestProjectionSentinel:
    ...
    def test_sentinel_importable_from_projection_module(self):
        ...
        assert "CANONICAL_RUNTIME_NODE_REGISTRY_ALIGNED" in src, (
            "CANONICAL_RUNTIME_NODE_REGISTRY_ALIGNED sentinel must be defined " "in core/routes/projection.py"
        )
                ...
                node_list.append(
                    {
                        ...
                        "status": (
                            m.status.name.lower() if hasattr(m.status, "name") else str(getattr(m, "status", "unknown"))
                        ),
