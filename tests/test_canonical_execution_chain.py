...
class TestChainStageAuthority:
    ...
    def test_device_router_stage_authority(self):
        ...
        assert CHAIN_STAGE_AUTHORITY[CanonicalChainStage.DEVICE_ROUTER_DISPATCH] == DEVICE_ROUTER_DISPATCH_AUTHORITY
...
class TestSidePathModuleRegistry:
    ...
    def test_no_authority_roles_in_side_path_registry(self):
        ...
        for module, role in SIDE_PATH_MODULE_REGISTRY.items():
            assert module not in authority_sentinels, f"Canonical authority {module} found in side-path registry"
...
class TestBuildChainContext:
    ...
    def test_returns_chain_execution_context(self):
        from core.canonical_execution_chain import ChainExecutionContext, build_chain_context
    ...
    def test_initial_stage_is_route_ingress(self):
        from core.canonical_execution_chain import CanonicalChainStage, build_chain_context
...
class TestChainExecutionContextAdvanceStage:
    ...
    def test_to_dict_contains_required_keys(self):
        ...
        assert expected_keys <= set(d.keys()), f"Missing keys in to_dict(): {expected_keys - set(d.keys())}"
...
class TestMainlineChainAuthorityUpdates:
    ...
    def test_existing_authorities_preserved(self):
        ...
        for authority_name in (
            ...
            assert hasattr(MainlineChainAuthority, authority_name), f"Existing authority {authority_name} missing"
...
class TestStageAuthorityMapping:
    ...
    def test_device_router_dispatch_mapped(self):
        ...
        assert STAGE_AUTHORITY.get(MainlineChainStage.DEVICE_ROUTER_DISPATCH) == (MainlineChainAuthority.DEVICE_ROUTER)
    ...
    def test_openclawd_authority_mapped(self):
        ...
        assert STAGE_AUTHORITY.get(MainlineChainStage.OPENCLAWD_AUTHORITY) == (MainlineChainAuthority.OPENCLAWD)
