...
TOOLS = [
    _tool("node__fs__read_file",
          params={"path": {"type": "string"}}, required=["path"]),
    _tool("node__fs__write_file",
          params={"path": {"type": "string"}, "content": {"type": "string"}},
          required=["path", "content"]),
    _tool("node__sys__set_volume",
...
class TestValidateToolCall:
    ...
    def test_enum_violation_rejected(self):
        v = validate_tool_call(
            "node__sys__set_volume", {"level": 50, "mode": "pct"}, TOOLS)
        assert not v.valid
        assert any("mode" in e for e in v.errors)

    def test_valid_call_passes(self):
        v = validate_tool_call(
            "node__sys__set_volume", {"level": 50, "mode": "abs"}, TOOLS)
        assert v.valid
...
class TestToolClassifiers:
    @pytest.mark.parametrize("name", [
        "node__fs__read_file", "node__sys__get_status", "mcp__mem__search_notes",
        "node__app__list_windows", "device__query_battery", "node__net__health",
    ])
    def test_read_only(self, name):
        assert is_read_only_tool(name)

    @pytest.mark.parametrize("name", [
        "node__fs__write_file", "node__app__launch", "node__sys__set_volume",
        "node__fs__delete_file", "完全未知的工具",
    ])
    def test_state_changing_or_unknown_conservative(self, name):
        assert not is_read_only_tool(name)

    @pytest.mark.parametrize("name", [
        "node__fs__delete_file", "node__sys__shutdown", "node__pkg__uninstall",
        "mcp__bank__transfer_funds", "node__mail__send_email",
    ])
...
class TestReactLoopInterception:
    ...
    async def test_invalid_args_not_dispatched_then_self_corrected(self):
                ...
                if n == 1:  # 第一轮:缺必填 content 的非法调用
                    return LLMResponse(
                        ...
                        tool_calls=[{"id": "c1", "type": "function",
                                     "function": {"name": "node__fs__write_file",
                                                  "arguments": '{"path": "/a"}'}}])
                    ...
                    return LLMResponse(
                        ...
                        tool_calls=[{"id": "c2", "type": "function",
                                     "function": {"name": "node__fs__write_file",
                                                  "arguments":
                                                  '{"path": "/a", "content": "hi"}'}}])
                return LLMResponse(content="写好了。", provider="p", model="m",
                                   input_tokens=1, output_tokens=1, latency_ms=1.0)
        ...
        result = await clawd._react_loop(
            [{"role": "user", "content": "写个文件"}], tools=TOOLS)

        assert result["response"] == "写好了。"
        # 非法那次没有派发;自纠后的那次才派发
        assert dispatched == [("node__fs__write_file",
                               {"path": "/a", "content": "hi"})]
        # 第二轮模型能看到结构化校验反馈(tool 消息,tool_call_id 对应 c1)
        fb = [m for m in rounds[1]
...
class TestShadowToolSimulator:
    ...
    async def test_simulator_llm_failure_falls_back_to_generic_shell(self):
        from core.liminal_rehearsal import ShadowState, ShadowToolSimulator
        sim = ShadowToolSimulator(None, _SimRouter(raise_on_chat=True),
                                  ShadowState(), TOOLS)
        out = await sim.simulate_call("node__fs__write_file",
                                      {"path": "/a", "content": "x"}, "写文件")
        assert out["simulated"] is True
...
class _RehearsalRouter:
    ...
    async def chat_with_tools(self, messages, tools=None, max_tokens=1024,
                              **kwargs):
        self.react_rounds += 1
        ...
        if not any(m.get("role") == "tool" for m in messages):
            r.tool_calls = [{"id": "s1", "type": "function",
                             "function": {"name": "node__fs__write_file",
                                          "arguments":
                                          '{"path": "/a", "content": "hi"}'}}]
        else:
    ...
    async def chat(self, messages, temperature=0.0, max_tokens=500, **kwargs):
        ...
        if "任务完成度裁判" in prompt:
            ...
            if self.judge_rounds == 1:   # 第一轮判未完成,给任务级反馈
                r.content = json.dumps({"complete": False,
                                        "feedback": "还差校验写入结果"},
                                       ensure_ascii=False)
            else:
...
class TestLiminalRehearsal:
    ...
    async def test_fail_feedback_retry_success_and_guidance(self):
        ...
        rl = LiminalRehearsal(router=router, real_dispatch=None, tools=TOOLS,
                              max_attempts=2, max_steps=4)
        ...
        assert any("上一轮预演反馈" in m.get("content", "")
                   and "还差校验写入结果" in m.get("content", "")
        ...
        class _Router(_RehearsalRouter):
            async def chat_with_tools(self, messages, tools=None,
                ...
                if self.react_rounds == 1:   # 缺必填 content
                    ...
                    assert fb and "参数校验失败" in fb[0]["content"]
                    r.tool_calls = [{"id": "s2", "type": "function",
                                     "function": {"name": "node__fs__write_file",
                                                  "arguments":
                                                  '{"path": "/a", "content": "hi"}'}}]
                else:
            ...
            async def chat(self, messages, **kwargs):
                ...
                class _R:
                    content = json.dumps({"complete": True}) \
                        if "任务完成度裁判" in prompt \
                        else json.dumps({"response": {"success": True},
                                         "state_delta": {}})
                return _R()

        rl = LiminalRehearsal(router=_Router(), real_dispatch=None, tools=TOOLS,
    ...
    async def test_semantic_reject_carries_reason(self):
        class _Router:
            async def chat(self, messages, **kwargs):
                class _R:
                    content = json.dumps(
                        {"ok": False, "reason": "要 ticker 不要公司名"},
                        ensure_ascii=False)
                return _R()

        v = await semantic_check("mcp__stock__delete_alert",
                                 {"symbol": "苹果公司"}, "删掉苹果的提醒",
                                 router=_Router())
        assert not v.valid
