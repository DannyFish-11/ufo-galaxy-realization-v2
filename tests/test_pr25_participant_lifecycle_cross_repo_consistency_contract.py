...
def test_authority_sentinel_and_version_present() -> None:
    ...
    assert len(ANDROID_AUDITED_REF) == SHA1_HEX_LENGTH
...
def test_android_capability_gates_partition_is_complete() -> None:
    assert ANDROID_FORMAL_CAPABILITY_ADVERTISEMENT_ALLOWED.isdisjoint(ANDROID_FORMAL_CAPABILITY_ADVERTISEMENT_BLOCKED)
    assert (
        ANDROID_FORMAL_CAPABILITY_ADVERTISEMENT_ALLOWED | ANDROID_FORMAL_CAPABILITY_ADVERTISEMENT_BLOCKED
    ) == ANDROID_FORMAL_WIRE_VALUES
...
def test_critical_trigger_semantic_mapping_count_and_uniqueness() -> None:
    ...
    keys = {(m.trigger.value, m.v2_from_state.value, m.v2_to_state.value) for m in CRITICAL_TRIGGER_SEMANTIC_MAPPINGS}
