...
def _wechat_tree() -> UIElementNode:
    return UIElementNode(role="window", label="微信", package="com.tencent.mm", children=[
...
class TestToPrompt:
    ...
    def test_vision_confidence_flagged(self):
        n = UIElementNode(role="button", label="悬浮球", source=UISource.VISION,
                          confidence=0.82, clickable=True)
...
class TestGraphMergeStructureAndVision:
    def test_vision_adds_missing_control(self):
        ...
        vg = UIGraph(root=UIElementNode(role="button", label="悬浮球", source=UISource.VISION,
                     confidence=0.82, clickable=True,
    ...
    def test_overlapping_vision_dedupes_and_supplies_label(self):
        ...
        struct = UIElementNode(role="window", children=[
            UIElementNode(role="button", label="", clickable=True,
                          bounds=UIBounds(x=1180, y=2020, width=120, height=80)),
        ])
        g = UIGraph(root=struct, source=UISource.UIA)
        vg = UIGraph(root=UIElementNode(role="button", label="发送", source=UISource.VISION,
                     confidence=0.9, bounds=UIBounds(x=1182, y=2022, width=118, height=78)),
                     source=UISource.VISION)
        merged = g.merge(vg)
    ...
    def test_merge_does_not_mutate_original(self):
        ...
        g.merge(UIGraph(root=UIElementNode(role="button", label="x", clickable=True,
                bounds=UIBounds(x=1, y=1, width=9, height=9)), source=UISource.VISION))
...
class TestBoundsIoU:
    ...
    def test_disjoint_iou_zero(self):
        assert UIBounds(x=0, y=0, width=10, height=10).iou(
            UIBounds(x=100, y=100, width=10, height=10)) == 0.0


class TestJsonRoundtrip:
    def test_graph_roundtrip_lossless(self):
        g = UIGraph(root=_wechat_tree(), source=UISource.ANDROID_A11Y,
                    app="com.tencent.mm", screen_width=1280, screen_height=2200)
