...
class TestCapabilityIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_capability_registration(self):
        ...
        success = await self.capability_manager.register_capability(
            name="test_http_get", description="HTTP GET 请求", node_id="08", node_name="Fetch", category="http"
        )
    ...
    async def test_capability_discovery(self):
        """测试能力发现"""
        # 注册多个能力
        await self.capability_manager.register_capability("test_cap_1", "测试能力1", "node1", "Node1", "test")
        await self.capability_manager.register_capability("test_cap_2", "测试能力2", "node2", "Node2", "test")
    ...
    async def test_capability_status_update(self):
        ...
        # 注册能力
        await self.capability_manager.register_capability("test_status", "状态测试", "node_test", "TestNode", "test")

        # 更新状态
        success = await self.capability_manager.update_capability_status("test_status", CapabilityStatus.OFFLINE)
    ...
    async def test_connection_registration(self):
        ...
        success = await self.connection_manager.register_connection(
            connection_id="test_node_08", url="http://localhost:8008"
        )
    ...
    async def test_connection_lifecycle(self):
        ...
        await self.connection_manager.register_connection(conn_id, "http://localhost:8000")
    ...
    async def asyncTearDown(self):
        """测试清理"""
        # 清理测试数据
        test_capabilities = ["test_http_get", "test_cap_1", "test_cap_2", "test_status"]
...
class TestSystemIntegration(unittest.TestCase):
    ...
    def test_imports(self):
        ...
        # 测试能力管理器导入
        try:
            from core.capability_manager import Capability, CapabilityManager, CapabilityStatus, get_capability_manager
