...
class TestPhaseA_HybridExecutorRename:
    ...
    def test_arbiter_uses_renamed_class(self):
        from core.hybrid_executor import AppExecutionCapabilityRegistry, HybridExecutionArbiter
    ...
    def test_register_and_get_round_trip(self):
        ...
        from core.hybrid_executor import AppCapability, AppExecutionCapabilityRegistry, ExecutionLevel
...
class TestPhaseA_OpenClawdToolCollection:
    ...
    def test_node_tools_collected_from_resolver_when_resolver_has_nodes(self):
        ...
        assert (
            "node__test_node__ping" in names
...
class TestPhaseA_SchedulerToolNaming:
    ...
    def test_inject_mcp_tools_emits_canonical_prefix(self):
        ...
        with patch("core.mcp_loader.mcp_loader", mock_loader), patch("core.mcp_loader.MCPServerStatus", _FakeStatus):
            sched.inject_mcp_tools()

        names = [t["function"]["name"] for t in sched.tools_cache]
        assert len(names) == 1
        assert (
            names[0] == "mcp__abcd1234__do_something"
        ...
        assert not any(
            n.startswith("mcp_abcd1234_") for n in names
    ...
    def test_inject_skill_tools_emits_canonical_prefix(self):
        ...
        names = [t["function"]["name"] for t in sched.tools_cache]
        assert len(names) == 1
        assert names[0] == "skill__hello_skill", f"Expected canonical 'skill__hello_skill', got {names[0]!r}"
...
class TestPhaseB_CallableNodeBaseline:
    ...
    def test_callable_classes_mirrors_capability_sync_eligible(self):
        ...
        eligible_values = {(c.value if hasattr(c, "value") else str(c)) for c in _CAPABILITY_SYNC_ELIGIBLE}
...
class TestPhaseC_PurgeRegistryEntries:
    ...
    def test_hybrid_executor_entry_pr_phase_a(self):
        ...
        entry = next(
            (e for e in PURGE_REGISTRY if e.asset_path == "core/hybrid_executor.py::CapabilityRegistry"),
            None,
        )
    ...
    def test_scheduler_entries_pr_phase_a(self):
        ...
        assert len(phase_a) >= 4, f"Expected ≥ 4 PR-consolidation-A purge entries, got {len(phase_a)}"
...
class TestPhaseA_NamingRoundTrip:
    ...
    def test_canonical_dispatcher_classifies_all_prefixes(self):
        ...
        for tool_name, expected in cases:
            ...
            assert got == expected, f"classify({tool_name!r}) → {got.value!r}, expected {expected.value!r}"
    ...
    def test_tool_call_schema_classifies_canonical_prefixes(self):
        ...
        for tool_name, expected in cases:
            ...
            assert got == expected, f"classify_layer({tool_name!r}) → {got.value!r}, expected {expected.value!r}"
