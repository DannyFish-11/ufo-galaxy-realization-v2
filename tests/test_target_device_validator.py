...
class TestRegisteredReadyDevice(unittest.TestCase):
    ...
    def test_valid_device_returns_valid_true(self):
        ...
        with patch(
            "core.target_device_validator._check_readiness", return_value=(True, True, {"readiness": rs.to_dict()}, [])
        ):
    ...
    def test_valid_device_no_reasons_for_failure(self):
        with patch("core.target_device_validator._check_readiness", return_value=(True, True, {}, [])):
            result = validate_target_device("dev-001")
...
class TestMissingDevice(unittest.TestCase):
    ...
    def test_registered_but_not_ready_returns_valid_false(self):
        ...
        with patch("core.target_device_validator._check_readiness", return_value=(True, False, {}, [])):
...
class TestCapabilityMismatch(unittest.TestCase):
    def test_missing_capability_returns_valid_false(self):
        with patch("core.target_device_validator._check_readiness", return_value=(True, True, {}, [])):
            with patch(
                ...
                return_value=(
                    False,
                    {"device_capabilities": ["screen", "touch"], "required": ["camera"]},
                    ["missing-capabilities:camera"],
                ),
            ):
                result = validate_target_device("dev-cap", required_capabilities=["camera"])
        self.assertFalse(result.valid)
    ...
    def test_missing_capability_reason_lists_missing_cap(self):
        with patch("core.target_device_validator._check_readiness", return_value=(True, True, {}, [])):
            with patch(
                ...
                result = validate_target_device("dev-cap", required_capabilities=["camera", "gps"])
        missing_reason = next((r for r in result.reasons if r.startswith("missing-capabilities")), None)
    ...
    def test_matching_capabilities_returns_valid_true(self):
        with patch("core.target_device_validator._check_readiness", return_value=(True, True, {}, [])):
            with patch(
                "core.target_device_validator._check_capabilities",
                return_value=(True, {"device_capabilities": ["screen", "touch", "camera"], "required": ["camera"]}, []),
            ):
                result = validate_target_device("dev-cap", required_capabilities=["camera"])
        self.assertTrue(result.valid)
        self.assertTrue(result.capability_match)
...
class TestCapabilitySubsystemDegradation(unittest.TestCase):
    def test_udm_import_error_does_not_crash(self):
        ...
        with patch("core.target_device_validator._check_readiness", return_value=(True, True, {}, [])):
            with patch(
                ...
                result = validate_target_device("dev-x", required_capabilities=["camera"])
    ...
    def test_udm_runtime_error_records_reason_no_crash(self):
        ...
        degraded_reason = next((r for r in result.reasons if r.startswith("capability-check-degraded:")), None)
    ...
    def test_udm_returns_none_skips_cap_check(self):
        ...
        skip_reason = next((r for r in result.reasons if r.startswith("capability-check-skipped:")), None)
    ...
    def test_orchestration_eligible_passes_when_required(self):
        with patch("core.target_device_validator._check_readiness", return_value=(True, True, {}, [])):
            with patch(
                ...
                result = validate_target_device("dev-orch", require_orchestration_eligible=True)
        self.assertTrue(result.valid)
        self.assertTrue(result.orchestration_eligible)
    ...
    def test_orchestration_module_unavailable_degrades_gracefully(self):
        ...
        orch_reason = next(
            (
                ...
                if r.startswith("orchestration-check-unavailable:") or r.startswith("orchestration-check-error:")
            ),
            None,
        )
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_sources_populated_from_readiness(self):
        with patch(
            "core.target_device_validator._check_readiness", return_value=(True, True, {"readiness": {"ok": True}}, [])
        ):
            result = validate_target_device("dev-x")
