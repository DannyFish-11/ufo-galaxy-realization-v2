...
class TestModuleImport:
    ...
    def test_claim_id_enum_covers_7_families(self) -> None:
        ...
        assert len(claim_ids) == 7, f"Expected 7 claim IDs; got {len(claim_ids)}: " f"{[c.value for c in claim_ids]}"

    def test_path_id_enum_covers_6_paths(self) -> None:
        path_ids = list(PostClosurePathId)
        assert len(path_ids) == 6, f"Expected 6 path IDs; got {len(path_ids)}"

    def test_closure_status_enum_covers_6_tiers(self) -> None:
        statuses = list(ClosureStatus)
        assert len(statuses) == 6, f"Expected 6 ClosureStatus tiers; got {len(statuses)}"

    def test_next_pr_priority_enum_covers_4_tiers(self) -> None:
        tiers = list(NextPRPriority)
        assert len(tiers) == 4, f"Expected 4 NextPRPriority tiers; got {len(tiers)}"
...
class TestClosurePRCatalog:
    ...
    def test_at_least_4_closure_prs_recorded(self, report: PostClosureReport) -> None:
        assert len(report.closure_prs) >= 4, f"Expected at least 4 closure PRs; got {len(report.closure_prs)}"
    ...
    def test_android_pr335_recorded(self, report: PostClosureReport) -> None:
        refs = [pr.pr_ref for pr in report.closure_prs]
        assert "Android#335" in refs, f"Android#335 (PR-C Android) not in closure PR catalog: {refs}"
    ...
    def test_pr1011_references_e2e_test_module(self, report: PostClosureReport) -> None:
        pr_a = next(pr for pr in report.closure_prs if pr.pr_ref == "V2#1011")
        assert any(
            "test_android_runtime_state_snapshot_e2e" in m for m in pr_a.v2_test_modules
        ), f"PR-A test modules: {pr_a.v2_test_modules}"

    def test_pr1013_references_delegated_closure_test(self, report: PostClosureReport) -> None:
        pr_b = next(pr for pr in report.closure_prs if pr.pr_ref == "V2#1013")
        assert any(
            "test_delegated_execution_runtime_closure" in m for m in pr_b.v2_test_modules
...
class TestClaimStatusCoverage:
    ...
    def test_all_items_are_post_closure_claim_status(self, report: PostClosureReport) -> None:
    ...
    def test_no_claim_is_surface_alignment_only(self, report: PostClosureReport) -> None:
        ...
        bad = [
            c.claim_id.value for c in report.claim_statuses if c.updated_label == ClosureStatus.SURFACE_ALIGNMENT_ONLY
        ]
        assert not bad, f"Unexpected SURFACE_ALIGNMENT_ONLY claims after closure: {bad}"

    def test_no_claim_is_obsolete_draft_noise(self, report: PostClosureReport) -> None:
        bad = [c.claim_id.value for c in report.claim_statuses if c.updated_label == ClosureStatus.OBSOLETE_DRAFT_NOISE]
    ...
    def test_android_carrier_claim_is_runtime_evidenced(self, report: PostClosureReport) -> None:
        carrier = next(
            c for c in report.claim_statuses if c.claim_id == PostClosureClaimId.ANDROID_IS_RUNTIME_CARRIER_NOT_CLIENT
        )
        assert carrier.updated_label == ClosureStatus.RUNTIME_EVIDENCED_CLOSED, (
            f"Expected RUNTIME_EVIDENCED_CLOSED for Android carrier claim; " f"got {carrier.updated_label}"
        )

    def test_system_beyond_poc_claim_is_runtime_evidenced(self, report: PostClosureReport) -> None:
        beyond_poc = next(c for c in report.claim_statuses if c.claim_id == PostClosureClaimId.SYSTEM_BEYOND_POC)
        assert beyond_poc.updated_label == ClosureStatus.RUNTIME_EVIDENCED_CLOSED, (
            f"Expected RUNTIME_EVIDENCED_CLOSED for beyond PoC claim; " f"got {beyond_poc.updated_label}"
        )

    def test_system_identity_claim_upgraded(self, report: PostClosureReport) -> None:
        identity = next(
            c for c in report.claim_statuses if c.claim_id == PostClosureClaimId.SYSTEM_IDENTITY_DISTRIBUTED_AI_BODY
        )
    ...
    def test_claim_statuses_have_prior_labels(self, report: PostClosureReport) -> None:
        for c in report.claim_statuses:
            assert isinstance(c.prior_label, str) and c.prior_label, f"Claim {c.claim_id.value} has empty prior_label"

    def test_claims_with_changed_true_have_closure_pr_refs(self, report: PostClosureReport) -> None:
        for c in report.claim_statuses:
            if c.changed:
                assert c.closure_pr_refs, (
                    f"Claim {c.claim_id.value} is marked changed=True " f"but has no closure_pr_refs"
                )
...
class TestPathStatusCoverage:
    ...
    def test_all_items_are_post_closure_path_status(self, report: PostClosureReport) -> None:
    ...
    def test_runtime_snapshot_uplink_is_closed(self, report: PostClosureReport) -> None:
        ...
        snap = next(p for p in report.path_statuses if p.path_id == PostClosurePathId.RUNTIME_SNAPSHOT_UPLINK)
        assert snap.runtime_closed is True, "RUNTIME_SNAPSHOT_UPLINK should be runtime_closed after PR-A"
        assert (
            snap.prior_label == "surface_alignment_only"
    ...
    def test_task_dispatch_result_is_closed(self, report: PostClosureReport) -> None:
        ...
        dispatch = next(p for p in report.path_statuses if p.path_id == PostClosurePathId.TASK_DISPATCH_EXECUTE_RESULT)
        assert dispatch.runtime_closed is True, "TASK_DISPATCH_EXECUTE_RESULT should be runtime_closed after PR-B"
        assert (
            dispatch.prior_label == "partially_established"
    ...
    def test_execution_event_uplink_is_closed(self, report: PostClosureReport) -> None:
        ...
        evt = next(p for p in report.path_statuses if p.path_id == PostClosurePathId.EXECUTION_EVENT_UPLINK)
        assert evt.runtime_closed is True, "EXECUTION_EVENT_UPLINK should be runtime_closed after PR-A + PR-B"

    def test_android_registration_is_closed(self, report: PostClosureReport) -> None:
        reg = next(p for p in report.path_statuses if p.path_id == PostClosurePathId.ANDROID_REGISTRATION)
        assert reg.runtime_closed is True, "ANDROID_REGISTRATION should be runtime_closed"

    def test_orchestration_path_is_closed_by_this_pr(self, report: PostClosureReport) -> None:
        ...
        assert orch.updated_label != ClosureStatus.STILL_MISSING_DECISION_PATH_CLOSURE, (
            f"ORCHESTRATION_CONSUMES_ANDROID_TRUTH should be closed; " f"got {orch.updated_label}"
        )
        assert (
            orch.prior_label == "surface_alignment_only"
        ...
        assert orch.closure_pr_refs, "ORCHESTRATION_CONSUMES_ANDROID_TRUTH must have closure_pr_refs"

    def test_orchestration_path_has_gap_description(self, report: PostClosureReport) -> None:
        orch = next(
            p for p in report.path_statuses if p.path_id == PostClosurePathId.ORCHESTRATION_CONSUMES_ANDROID_TRUTH
        )
        assert orch.gap_description, "ORCHESTRATION_CONSUMES_ANDROID_TRUTH must have a gap_description"
    ...
    def test_at_least_4_paths_runtime_closed(self, report: PostClosureReport) -> None:
        ...
        assert len(closed) >= 4, (
            f"Expected at least 4 paths runtime_closed; got {len(closed)}: " f"{[p.path_id.value for p in closed]}"
        )
    ...
    def test_path_statuses_have_prior_labels(self, report: PostClosureReport) -> None:
        for p in report.path_statuses:
            assert isinstance(p.prior_label, str) and p.prior_label, f"Path {p.path_id.value} has empty prior_label"

    def test_closed_paths_reference_closure_prs(self, report: PostClosureReport) -> None:
        for p in report.path_statuses:
            if p.runtime_closed:
                assert p.closure_pr_refs, f"Path {p.path_id.value} is runtime_closed but has no closure_pr_refs"
...
class TestNextPRRoadmap:
    ...
    def test_at_least_1_p0_item(self, report: PostClosureReport) -> None:
        ...
        assert len(p0_items) >= 1, f"Expected at least 1 P0 next-PR item; got {len(p0_items)}"

    def test_p0_item_targets_orchestration_decision_path(self, report: PostClosureReport) -> None:
        p0_items = [n for n in report.next_pr_items if n.priority == NextPRPriority.P0_DECISION_PATH_CLOSURE]
        p0 = p0_items[0]
        # The P0 item must clearly reference orchestration/routing/dispatch
        combined = (p0.title + " " + p0.rationale).lower()
        assert any(kw in combined for kw in ["orchestration", "routing", "dispatch", "decision"]), (
            f"P0 item title/rationale should reference orchestration/routing/dispatch. " f"Got title='{p0.title}'"
        )
    ...
    def test_p0_item_references_v2_repo(self, report: PostClosureReport) -> None:
        ...
        p0 = p0_items[0]
        assert any(
            "ufo-galaxy-realization-v2" in r for r in p0.target_repos
...
class TestAggregateCounts:
    ...
    def test_claim_bucket_sum_equals_total_claims(self, report: PostClosureReport) -> None:
        total = (
        ...
        assert total == len(
            report.claim_statuses
        ), f"Claim bucket sum {total} != len(claim_statuses) {len(report.claim_statuses)}"

    def test_path_closed_plus_open_equals_total_paths(self, report: PostClosureReport) -> None:
        total = report.paths_runtime_closed + report.paths_still_open
        assert total == len(report.path_statuses), (
            f"paths_runtime_closed + paths_still_open {total} != " f"len(path_statuses) {len(report.path_statuses)}"
        )

    def test_paths_runtime_closed_is_at_least_4(self, report: PostClosureReport) -> None:
        assert (
            report.paths_runtime_closed >= 4
    ...
    def test_claims_upgraded_is_at_least_5(self, report: PostClosureReport) -> None:
        assert report.claims_upgraded >= 5, f"Expected at least 5 upgraded claims; got {report.claims_upgraded}"

    def test_strongly_established_plus_runtime_evidenced_is_majority(self, report: PostClosureReport) -> None:
        strong = report.strongly_established_count + report.runtime_evidenced_closed_count
        assert strong >= 5, (
            f"Expected at least 5 claims to be strongly established or " f"runtime evidenced closed; got {strong}"
        )
...
class TestChineseConclusion:
    ...
    def test_verdict_zh_mentions_pr_a_number(self) -> None:
        assert "1011" in POST_CLOSURE_DUAL_REPO_VERDICT_ZH, "Chinese verdict should mention PR #1011 (PR-A)"

    def test_verdict_zh_mentions_pr_b_number(self) -> None:
        assert "1013" in POST_CLOSURE_DUAL_REPO_VERDICT_ZH, "Chinese verdict should mention PR #1013 (PR-B)"

    def test_verdict_zh_mentions_pr_c_number(self) -> None:
        assert "1015" in POST_CLOSURE_DUAL_REPO_VERDICT_ZH, "Chinese verdict should mention PR #1015 (PR-C V2)"

    def test_verdict_zh_mentions_android_pr335(self) -> None:
        assert "335" in POST_CLOSURE_DUAL_REPO_VERDICT_ZH, "Chinese verdict should mention Android #335 (PR-C Android)"

    def test_verdict_zh_mentions_orchestration_gap(self) -> None:
        combined = POST_CLOSURE_DUAL_REPO_VERDICT_ZH.lower()
        assert any(
            kw in combined for kw in ["orchestration", "routing", "dispatch", "决策", "路由", "分发"]
...
class TestSingletonCaching:
    ...
    def test_singleton_is_thread_safe(self) -> None:
        ...
        assert all(r is results[0] for r in results), "All threads should receive the same cached instance"
...
class TestComparisonWithPriorReevaluation:
    ...
    def test_snapshot_path_was_upgraded_from_prior(self, report: PostClosureReport) -> None:
        ...
        assert (
            snap.updated_label != ClosureStatus.SURFACE_ALIGNMENT_ONLY
