...
def start_node(node: dict) -> subprocess.Popen:
    ...
    process = subprocess.Popen(
        [sys.executable, str(main_py)], cwd=str(node_dir), env=env, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        try:
            response = await client.post(
                ...
                json={"node_id": "test", "resource_id": "test_resource", "timeout_seconds": 10},
            )
            ...
            if result.get("success"):
                ...
                await client.post(
                    ...
                    json={"node_id": "test", "resource_id": "test_resource", "token": result.get("token")},
                )
            else:
        ...
        print("\n📋 Test 3: Model Routing - Simple Query")
        try:
            response = await client.post("http://localhost:8058/route", json={"prompt": "What time is it?"})
            result = response.json()
        ...
        try:
            response = await client.post(
                ...
                json={"action": "adb_screenshot", "target_node": "33", "params": {}, "source_node": "test"},
            )
            result = response.json()
            if result.get("success"):
        ...
        try:
            ...
            successes = sum(1 for r in responses if not isinstance(r, Exception) and r.json().get("success"))
