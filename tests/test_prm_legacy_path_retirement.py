...
class TestPRMStatus:
    ...
    def test_smart_transport_router_is_legacy_compat(self):
        ...
        entry = LEGACY_PATH_REGISTRY["galaxy_gateway.smart_transport_router.SmartTransportRouter"]
    ...
    def test_enhanced_nlu_engine_v2_is_legacy_compat(self):
        ...
        entry = LEGACY_PATH_REGISTRY["galaxy_gateway.enhanced_nlu_v2.EnhancedNLUEngineV2"]
        assert entry.status is LegacyPathStatus.LEGACY_COMPATIBILITY
    ...
    def test_session_roaming_manager_is_legacy_compat(self):
        ...
        entry = LEGACY_PATH_REGISTRY["galaxy_gateway.session_roaming.SessionRoamingManager"]
        assert entry.status is LegacyPathStatus.LEGACY_COMPATIBILITY
...
class TestLegacyOrchestratorPathsShimPRM:
    ...
    def test_shim_includes_smart_transport_router(self):
        ...
        assert "galaxy_gateway.smart_transport_router.SmartTransportRouter" in LEGACY_ORCHESTRATOR_PATHS

    def test_shim_includes_enhanced_nlu_engine_v2(self):
        from core.orchestration_authority.legacy_paths import LEGACY_ORCHESTRATOR_PATHS

        assert "galaxy_gateway.enhanced_nlu_v2.EnhancedNLUEngineV2" in LEGACY_ORCHESTRATOR_PATHS

    def test_shim_includes_session_roaming_manager(self):
        from core.orchestration_authority.legacy_paths import LEGACY_ORCHESTRATOR_PATHS

        assert "galaxy_gateway.session_roaming.SessionRoamingManager" in LEGACY_ORCHESTRATOR_PATHS
...
class TestIsLegacyPathPRM:
    def test_smart_transport_router_is_legacy(self):
        ...
        assert is_legacy_path("galaxy_gateway.smart_transport_router.SmartTransportRouter")
    ...
    def test_canonical_e2e_orchestrator_is_not_legacy_compat(self):
        ...
        entry = LEGACY_PATH_REGISTRY.get("core.e2e_orchestrator.process_user_input")
        if entry is not None:
            assert entry.status is not LegacyPathStatus.LEGACY_COMPATIBILITY, (
...
class TestPurgeRegistryPRM:
    ...
    def test_purge_registry_has_smart_transport_router_entry(self):
        ...
        assert any(
            "SmartTransportRouter" in p for p in paths
    ...
    def test_purge_registry_has_enhanced_nlu_engine_v2_entry(self):
        ...
        paths = {e.asset_path for e in PURGE_REGISTRY}
        assert any(
            "EnhancedNLUEngineV2" in p for p in paths
    ...
    def test_purge_registry_has_session_roaming_manager_entry(self):
        ...
        paths = {e.asset_path for e in PURGE_REGISTRY}
        assert any(
            "SessionRoamingManager" in p for p in paths
    ...
    def test_prm_entries_are_wrapper_hardened(self):
        ...
        for entry in prm:
            assert entry.status is PurgeStatus.WRAPPER_HARDENED, (
                f"PR-M entry {entry.asset_path!r} should be WRAPPER_HARDENED, " f"got {entry.status!r}"
            )
...
class TestPriorBatchRegressionGuard:
    ...
    def test_prs5_paths_still_registered(self):
        ...
        assert prs5_paths.issubset(LEGACY_ORCHESTRATOR_PATHS), "PR-S5 paths must still be present as regression guard"
    ...
    def test_prs6_paths_still_registered(self):
        ...
        assert prs6_paths.issubset(LEGACY_ORCHESTRATOR_PATHS), "PR-S6 paths must still be present as regression guard"
    ...
    def test_prs7_paths_still_registered(self):
        ...
        assert prs7_paths.issubset(LEGACY_ORCHESTRATOR_PATHS), "PR-S7 paths must still be present as regression guard"
