...
def _make_formation_group(
    *,
    ...
    members = []
    if source_id:
        members.append(FormationMember(device_id=source_id, role=FormationRole.SOURCE, reason="source"))
    if primary_id:
        members.append(FormationMember(device_id=primary_id, role=FormationRole.PRIMARY_EXECUTION, reason="primary"))
...
class TestEvaluateCapabilityGate:
    def test_pass_with_capabilities_attr(self):
        from core.capability_routing_gate import CapabilityGateVerdict, evaluate_capability_gate
    ...
    def test_custom_capability_attr(self):
        ...
        result = evaluate_capability_gate(device, ["nfc"], capability_attr="caps_list")
        assert result.verdict == CapabilityGateVerdict.PASS
...
class TestFilterByRequiredCapabilities:
    ...
    def test_insufficient_data_kept_by_default(self):
        ...
        result = filter_by_required_capabilities([dev_a, dev_b], ["camera"], allow_insufficient_data=True)
        ids = {d.device_id for d in result}
    ...
    def test_insufficient_data_excluded_when_flag_false(self):
        ...
        dev_b = _make_device("b", capabilities=["camera"])
        result = filter_by_required_capabilities([dev_a, dev_b], ["camera"], allow_insufficient_data=False)
        ids = {d.device_id for d in result}
        assert "a" not in ids
        assert "b" in ids
...
class TestTriggerRebalanceIfNeeded:
    ...
    def test_trigger_with_health_map(self):
        ...
        coordinator = _make_coordinator(group)
        health_map = {
            "primary-dev": FormationHealthSignal(device_id="primary-dev", health_score=0.0, is_reachable=False),
...
class TestAcceptanceCriterion1DynamicReevaluation:
    ...
    def test_primary_lost_reshapes_formation(self):
        ...
        health_map = {
            "primary-dev": FormationHealthSignal(device_id="primary-dev", health_score=0.0, is_reachable=False),
            "fb-promote": FormationHealthSignal(device_id="fb-promote", health_score=0.9, is_reachable=True),
        }
        ...
        if result.reshaped:
            new_primaries = [m.device_id for m in result.new_group.members if m.role == FormationRole.PRIMARY_EXECUTION]
...
class TestAcceptanceCriterion2CapabilityConstraint:
    ...
    def test_only_capable_devices_pass(self):
        ...
        result = filter_by_required_capabilities([capable, incapable, partial], ["camera", "gps"])
        ids = {d.device_id for d in result}
        assert ids == {"capable"}, "Only the device satisfying ALL required capabilities should be selected."
...
class TestAcceptanceCriterion3TransportLayerPurity:
    ...
    def test_device_router_docstring_declares_non_responsibilities(self):
        ...
        router_path = pathlib.Path(__file__).parent.parent / "galaxy_gateway" / "device_router.py"
        source = router_path.read_text(encoding="utf-8")

        # Check that the module docstring names 'Formation' and 'orchestration eligibility'
        # as non-responsibilities.
        assert (
            "Formation" in source or "formation" in source
        ...
        assert "eligibility" in source or "Orchestration eligibility" in source, (
            "DeviceRouter module should document orchestration eligibility " "as NOT its responsibility"
        )
...
class TestAcceptanceCriterion4TruthLayerBoundaries:
    ...
    def test_rebalance_engine_is_authority_sentinel(self):
        ...
        assert (
            "formation-rebalance" in FORMATION_REBALANCE_ENGINE_IS_AUTHORITY.lower()
            or "rebalance" in FORMATION_REBALANCE_ENGINE_IS_AUTHORITY.lower()
        )
