...
class TestLauncherConfigManagerDeprecation:
    ...
    def test_module_file_has_warnings_import(self):
        ...
        assert "import warnings" in src, "launcher/config_manager.py must import the `warnings` module"

    def test_module_file_emits_deprecation_warning_on_import(self):
        src = _read("launcher/config_manager.py")
        assert "warnings.warn" in src, "launcher/config_manager.py must call warnings.warn()"
        assert "DeprecationWarning" in src, "launcher/config_manager.py must pass DeprecationWarning to warnings.warn()"

    def test_module_header_still_says_deprecated(self):
        src = _read("launcher/config_manager.py")
        assert "DEPRECATED" in src, "launcher/config_manager.py module header must retain DEPRECATED notice"

    def test_deprecation_warning_mentions_canonical_replacement(self):
        src = _read("launcher/config_manager.py")
        assert (
            "core.unified.config_manager" in src or "UnifiedConfigManager" in src
    ...
    def test_import_emits_deprecation_warning_at_runtime(self):
        ...
        assert dep_warnings, "Importing launcher.config_manager must emit a DeprecationWarning"
...
class TestConfigAuthorityDocs:
    ...
    def test_launcher_config_manager_demoted_d2(self):
        src = _read("docs/CONFIGURATION_AUTHORITY.md")
        assert (
            "launcher/config_manager.py" in src
        ), "docs/CONFIGURATION_AUTHORITY.md must mention launcher/config_manager.py"
        assert (
            "D2" in src or "HARD_DEPRECATED" in src or "Deprecated" in src
...
class TestLegacySurfaceInventoryUpdated:
    ...
    def test_compat_routes_pr3_status_marked(self):
        ...
        assert any(
            "PR-3" in l or "✅" in l for l in lines
...
class TestStructuralInvariants:
    ...
    def test_launcher_config_manager_still_functional(self):
        ...
        assert (
            "class ConfigManager" in src
        ), "launcher/config_manager.py must still export ConfigManager for backward compat"
        assert "class NodeConfig" in src, "launcher/config_manager.py must still export NodeConfig for backward compat"
