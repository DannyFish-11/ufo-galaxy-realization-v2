...
def test_b03_snapshot_total_edges_reflects_registered():
    rt = get_network_topology_runtime()
    rt.register_edge(TopologyEdge(edge_id="se1", source_node_id="a", target_node_id="b"))
    rt.register_edge(TopologyEdge(edge_id="se2", source_node_id="c", target_node_id="d"))
    snap = rt.snapshot()
...
def test_b07_snapshot_edges_by_kind_reflects_kinds():
    rt = get_network_topology_runtime()
    rt.register_edge(TopologyEdge(edge_id="ek1", source_node_id="a", target_node_id="b", kind=TopologyEdgeKind.DIRECT))
    rt.register_edge(TopologyEdge(edge_id="ek2", source_node_id="c", target_node_id="d", kind=TopologyEdgeKind.RELAY))
    snap = rt.snapshot()
...
def test_c02_renderer_reads_device_nodes():
    rt = get_network_topology_runtime()
    rt.absorb_device_connectivity(device_id="dev-r", preferred_path="direct_ws", effective_routable=True)
...
def test_d03_device_absorption_visible_in_snapshot():
    rt = get_network_topology_runtime()
    rt.absorb_device_connectivity(device_id="dev-vis", preferred_path="direct_ws", effective_routable=True)
    snap = rt.snapshot()
    assert "device" in snap.nodes_by_kind
...
def test_d04_state_change_visible_in_snapshot():
    rt = get_network_topology_runtime()
    rt.register_node(TopologyNode(node_id="chg1", state=TopologyConnectionState.LATENT))
...
def test_d05_edge_removal_visible_in_snapshot():
    rt = get_network_topology_runtime()
    rt.register_edge(TopologyEdge(edge_id="er-vis", source_node_id="a", target_node_id="b"))
...
def test_f01_nodes_from_multiple_sources_coexist():
    ...
    rt.absorb_device_connectivity(device_id="dev-ms", preferred_path="direct_ws", effective_routable=True)
    nodes = rt.all_nodes()
...
def test_f02_snapshot_nodes_by_kind_reflects_multiple_sources():
    ...
    rt.absorb_device_connectivity(device_id="dev-m2", preferred_path="relay", effective_routable=True)
    snap = rt.snapshot()
    assert "nats_fabric_endpoint" in snap.nodes_by_kind
    assert "gateway" in snap.nodes_by_kind
    assert "device" in snap.nodes_by_kind
...
def test_f03_single_snapshot_covers_all_sources():
    ...
    rt.absorb_device_connectivity(device_id="dev-all", preferred_path="", effective_routable=False)
    snap = rt.snapshot()
