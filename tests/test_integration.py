...
from core.galaxy_main_loop_l4_enhanced import GalaxyMainLoopL4Enhanced, PendingGoal, get_galaxy_loop
from integration.event_bus import EventBus, EventType, UIGalaxyEvent, event_bus, ui_progress_callback
from system_integration.state_machine_ui_integration import (
    HardwareTriggerManager,
    SystemState,
    SystemStateMachine,
    TriggerType,
    hardware_trigger_manager,
    state_machine,
)
...
class TestEventBus(unittest.TestCase):
    ...
    def test_event_creation(self):
        """测试事件创建"""
        event = UIGalaxyEvent(event_type=EventType.GOAL_SUBMITTED, source="test", data={"test": "data"})
    ...
    def test_event_to_dict(self):
        """测试事件转换为字典"""
        event = UIGalaxyEvent(event_type=EventType.GOAL_SUBMITTED, source="test", data={"goal_id": "123"})
...
class TestL4Integration(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.galaxy_loop = GalaxyMainLoopL4Enhanced({"cycle_interval": 0.1, "auto_scan_interval": 300.0})
...
class TestStateMachineIntegration(unittest.TestCase):
    ...
    def test_wakeup(self):
        """测试唤醒（硬件触发 → UI 集成点）"""
        success = self.state_machine.wakeup(trigger_type=TriggerType.HARDWARE_BUTTON, trigger_source="test_button")
...
class TestHardwareTriggerManager(unittest.TestCase):
    ...
    def test_hardware_button_trigger(self):
        ...
        # 系统应该被唤醒
        self.assertEqual(self.trigger_manager.state_machine.current_state, SystemState.ISLAND)
    ...
    def test_gesture_trigger(self):
        ...
        self.assertEqual(self.trigger_manager.state_machine.current_state, SystemState.SIDESHEET)
...
class TestProgressCallback(unittest.TestCase):
    ...
    def test_goal_decomposition_callbacks(self):
        ...
        ui_progress_callback.on_goal_decomposition_completed("测试目标", [{"id": 1, "description": "子任务1"}])
    ...
    def test_action_execution_callbacks(self):
        ...
        ui_progress_callback.on_action_execution_completed("action_1", True, {"output": "success"})
