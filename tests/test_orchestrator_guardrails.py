...
class TestWarnLegacyPathEmitsWarning:
    ...
    def test_emits_guardrail_keyword(self, caplog):
        ...
        assert any(
            "LEGACY PATH GUARDRAIL" in m for m in messages
...
class TestWarnLegacyPathIncludesCaller:
    ...
    def test_caller_appears_in_log(self, caller: str, caplog):
        ...
        assert caller in messages, f"warn_legacy_path() must include caller={caller!r} in log output"
...
class TestWarnLegacyPathIncludesTraceId:
    ...
    def test_trace_id_in_log(self, caplog):
        ...
        messages = " ".join(r.getMessage() for r in caplog.records)
        assert trace in messages, "warn_legacy_path() must include the trace_id in the log output"
...
class TestE2EOrchestratorGuardrail:
    ...
    async def test_guardrail_not_fired_on_default(self, caplog):
        ...
        assert not guardrail_msgs, "No guardrail warning should fire when use_constellation=True (default)"
...
class TestAllLegacyPathsTriggerGuardrail:
    ...
    def test_all_paths_log_without_raising(self, caplog):
        ...
        guardrail_records = [r for r in caplog.records if "LEGACY PATH GUARDRAIL" in r.getMessage()]
        assert len(guardrail_records) >= len(LEGACY_ORCHESTRATOR_PATHS), (
            f"Expected at least {len(LEGACY_ORCHESTRATOR_PATHS)} guardrail records, " f"got {len(guardrail_records)}"
        )
