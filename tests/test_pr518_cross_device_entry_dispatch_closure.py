...
class TestB_CommandRouterCrossDevicePath(unittest.TestCase):
    ...
    def test_B4_empty_targets_allowed_for_cross_device(self):
        ...
        self.assertGreater(
            idx_pool, idx_bypass, "DevicePoolManager pool check should not apply to cross_device envelopes"
        )
    ...
    def test_B6_substrate_caller_key_propagated_to_device_router(self):
        ...
        from core.command_router import _CROSS_DEVICE_SUBSTRATE_CALLER_KEY, CommandRouter
...
class TestC_CoordinatorSubstrateGuard(unittest.TestCase):
    ...
    def test_C2_non_canonical_call_logs_legacy_dispatch_warning(self):
        ...
        with (
            ...
            patch.object(
                coord,
                "_execute_generic_cross_device_task",
                new_callable=AsyncMock,
                return_value={"success": True},
            ),
        ):
            with self.assertLogs("galaxy_gateway.cross_device_coordinator", level="WARNING") as cm:
                asyncio.new_event_loop().run_until_complete(coord.execute_cross_device_task("test command"))
            self.assertTrue(
    ...
    def test_C3_canonical_call_suppresses_warning(self):
        ...
        with (
            ...
            with self.assertNoLogs("galaxy_gateway.cross_device_coordinator", level="WARNING"):
...
class TestD_DeviceRouterSubstrateGuard(unittest.TestCase):
    ...
    def test_D2_no_substrate_caller_logs_legacy_warning(self):
        ...
        with (
            patch("galaxy_gateway.device_router.is_cross_device_enabled", return_value=True),
            patch.object(router, "_decompose_task", return_value=[task]),
            patch.object(
                router,
                "dispatch_task",
                new_callable=AsyncMock,
            ...
            with self.assertLogs("galaxy_gateway.device_router", level="WARNING") as cm:
                asyncio.new_event_loop().run_until_complete(router._dispatch_cross_device_task(task, [dev]))
            self.assertTrue(
                any("LEGACY_DISPATCH" in msg for msg in cm.output),
                f"Expected LEGACY_DISPATCH warning; got: {cm.output}",
            )
