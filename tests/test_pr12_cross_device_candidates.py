...
class TestResolveCrossDeviceCandidates:
    ...
    def test_device_excluded_when_readiness_fails(self):
        ...
        with _patch_enumerate(["dev-e"]), _patch_readiness({"dev-e": False}), _patch_participation({"dev-e": False}):
            result = resolve_cross_device_candidates()
    ...
    def test_no_capabilities_skips_capability_gate(self):
        ...
        with _patch_enumerate(["dev-f"]), _patch_readiness({"dev-f": True}), _patch_participation({"dev-f": True}):
            result = resolve_cross_device_candidates()
...
class TestRequestedTargetDevice:
    ...
    def test_target_not_in_registry_evaluated_with_reasons(self):
        ...
        with _patch_enumerate([]), _patch_readiness({}), _patch_participation({}):
    ...
    def test_target_in_registry_but_not_ready_other_devices_returned(self):
        ...
        with (
            _patch_enumerate(["dev-target", "dev-good"]),
...
class TestOrchestrationGateDisabled:
    def test_device_passes_when_orchestration_gate_skipped(self):
        ...
        with _patch_enumerate(["dev-g"]), _patch_readiness({"dev-g": True}), _patch_participation({"dev-g": False}):
    ...
    def test_readiness_still_required_when_orchestration_gate_off(self):
        ...
        with _patch_enumerate(["dev-h"]), _patch_readiness({"dev-h": False}), _patch_participation({"dev-h": True}):
            result = resolve_cross_device_candidates(require_orchestration_eligible=False)
...
class TestGetSelectedCrossDeviceCandidates:
    ...
    def test_returns_empty_when_none_eligible(self):
        ...
        with _patch_enumerate(["dev-x"]), _patch_readiness({"dev-x": False}):
            result = get_selected_cross_device_candidates()
        assert result == []
...
class TestGracefulDegradation:
    def test_readiness_layer_unavailable_no_crash(self):
        ...
        with _patch_enumerate(["dev-i"]), patch(f"{MOD}._check_readiness", side_effect=_raise):
            result = resolve_cross_device_candidates()
...
class TestObservability:
    def test_excluded_device_produces_info_log(self, caplog):
        ...
        with (
            ...
            resolve_cross_device_candidates()
        assert any("EXCLUDED" in r.message for r in caplog.records if r.name == "Galaxy.CrossDeviceCandidates")
    ...
    def test_selected_device_produces_debug_log(self, caplog):
        ...
        with (
            ...
            resolve_cross_device_candidates()
        assert any("SELECTED" in r.message for r in caplog.records if r.name == "Galaxy.CrossDeviceCandidates")
