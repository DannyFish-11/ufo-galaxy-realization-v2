...
class TestCapabilityOrchestratorWiring:
    ...
    @pytest.mark.asyncio
    async def test_R01_orchestrator_builtin_cross_device_sets_canonical_dispatch_path(self, _builtin_cross_device_cap):
        ...
        assert captured_context.get("dispatch_path") == DISPATCH_PATH_CANONICAL, (
            "CapabilityOrchestrator must pass dispatch_path=DISPATCH_PATH_CANONICAL " "to DeviceRouter (PR-02)"
        )
    ...
    @pytest.mark.asyncio
    async def test_R03_orchestrator_compat_fallback_sets_compat_route_mode(self, _builtin_cross_device_cap):
