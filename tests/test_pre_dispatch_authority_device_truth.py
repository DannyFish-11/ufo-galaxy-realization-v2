...
class TestCreateTaskCanonicalDispatch:
    ...
    async def test_canonical_dispatch_sets_authority(self):
        ...
        with (
            patch("core.routes.tasks.connection_manager", mock_conn),
...
class TestCompatWsUdmFirst:
    ...
    def test_udm_patch_called_on_connect_code_path(self):
        ...
        assert udm_pos < compat_pos, "UDM write must precede the COMPAT_MIRROR_WRITE label in the connect code path"
    ...
    def test_compat_mirror_write_labels_present_in_api_routes(self):
        ...
        assert count >= 5, f"Expected at least 5 COMPAT_MIRROR_WRITE labels in compat WS handler, got {count}"
...
class TestCompatRoutesUdmFirst:
    ...
    def test_heartbeat_udm_before_cache(self):
        ...
        assert compat_pos_after_udm > 0, "COMPAT_MIRROR_WRITE label must appear after udm_write_heartbeat"
    ...
    def test_unregister_udm_before_cache(self):
        ...
        assert compat_pos > 0, "COMPAT_MIRROR_WRITE label must appear after UDM patch in legacy unregister handler"
    ...
    def test_register_compat_mirror_write_label(self):
        ...
        src = inspect.getsource(compat_mod)
        assert "COMPAT_MIRROR_WRITE" in src, "COMPAT_MIRROR_WRITE labels must be present in core.routes.compat"
...
class TestGatewayWsCompatMirrorLabels:
    ...
    def test_compat_mirror_label_present_on_disconnect(self):
        ...
        assert "COMPAT_MIRROR_WRITE" in src, "COMPAT_MIRROR_WRITE labels must be present in websocket_handler"
    ...
    def test_compat_mirror_label_present_on_register(self):
        ...
        assert src.count("COMPAT_MIRROR_WRITE") >= 2, (
            "Expected at least 2 COMPAT_MIRROR_WRITE labels in websocket_handler " "(disconnect + register cache write)"
        )
