# ---------------------------------------------------------------------------
# CANONICAL LOCATION: tests/integration/test_nlu_v2.py
# Relocated from galaxy_gateway/test_nlu_v2.py
#
# NOTE: This is a manual NLU evaluation script that requires a live Galaxy
# instance with NLU services active.  It is marked ``manual`` so that it is
# skipped in standard CI runs.  Run explicitly with:
#   pytest tests/integration/test_nlu_v2.py -m manual
# ---------------------------------------------------------------------------
"""
...
try:
    from galaxy_gateway.enhanced_nlu_v2 import DeviceRegistry, EnhancedNLUEngineV2, LLMClient
except ImportError as _e:
    pytest.skip(f"galaxy_gateway NLU module not available: {_e}", allow_module_level=True)
...
TEST_CASES = [
    # 基础测试 - 单设备单应用
    {
        "category": "基础操作",
        "input": "在手机A上打开微信",
        "expected": {"devices": ["phone_a"], "actions": ["open"], "targets": ["wechat"]},
    },
    {
        "category": "基础操作",
        "input": "在平板上播放音乐",
        "expected": {"devices": ["tablet"], "actions": ["play"], "targets": ["music"]},
    },
    {
        "category": "基础操作",
        "input": "在电脑上打开Chrome",
        "expected": {"devices": ["pc"], "actions": ["open"], "targets": ["chrome"]},
    },
    # 多设备并行操作
    {
        "category": "多设备并行",
        "input": "在手机B上打开微信，在平板上播放音乐",
        "expected": {"devices": ["phone_b", "tablet"], "actions": ["open", "play"], "targets": ["wechat", "music"]},
    },
    {
        "category": "多设备并行",
    ...
    # 复杂任务 - 多步骤
    {
        "category": "复杂任务",
        "input": "把手机A上的照片发到电脑",
        "expected": {"devices": ["phone_a", "pc"], "task_count": 2, "has_dependencies": True},  # 读取 + 写入
    },
    {
        "category": "复杂任务",
        "input": "把手机上的照片发到电脑，然后用PS打开",
        "expected": {"devices": ["phone_a", "pc"], "task_count": 3, "has_dependencies": True},  # 读取 + 写入 + 打开
    },
    {
        "category": "复杂任务",
        "input": "在电脑上打开Chrome并搜索Python教程",
        "expected": {"devices": ["pc"], "task_count": 2, "has_dependencies": True},  # 打开 + 搜索
    },
    # 设备别名测试
    {
        "category": "设备别名",
    ...
    },
    {
        "category": "设备别名",
        "input": "在工作手机上打开QQ",
        "expected": {"devices": ["phone_b"], "actions": ["open"], "targets": ["qq"]},  # "工作手机" 是 phone_b 的别名
    },
    # 模糊指令（需要澄清）
    {"category": "模糊指令", "input": "打开微信", "expected": {"need_clarification": True}},  # 没有指定设备
    {
        "category": "模糊指令",
        "input": "在手机上打开那个聊天软件",
        "expected": {"need_clarification": True},  # "那个聊天软件" 不明确
    },
    # 自然语言多样性
    {
        "category": "自然语言多样性",
        "input": "帮我在手机B上启动微信",
        "expected": {"devices": ["phone_b"], "actions": ["open"], "targets": ["wechat"]},
    },
    {
        "category": "自然语言多样性",
        "input": "用平板播放一首歌",
    ...
    },
    {
        "category": "自然语言多样性",
        "input": "电脑上的浏览器打开一下",
        "expected": {"devices": ["pc"], "actions": ["open"], "targets": ["browser"]},
    },
]
...
class NLUTester:
    ...
    def __init__(self):
        ...
        self.rule_engine = EnhancedNLUEngineV2(
            device_registry=self.device_registry, llm_client=self.llm_client, use_llm=False  # 只用规则
        )

        self.llm_engine = EnhancedNLUEngineV2(
            device_registry=self.device_registry, llm_client=self.llm_client, use_llm=True  # 使用 LLM
        )
    ...
    async def run_tests(self) -> Dict[str, Any]:
        ...
        for i, test_case in enumerate(TEST_CASES, 1):
            ...
            # 测试规则引擎
            print(f"\n[规则引擎]")
            rule_result = await self._test_single(self.rule_engine, test_case, "rule")
            ...
            # 测试 LLM 引擎
            print(f"\n[LLM 引擎]")
            llm_result = await self._test_single(self.llm_engine, test_case, "llm")
    ...
    async def _test_single(
        self, engine: EnhancedNLUEngineV2, test_case: Dict[str, Any], engine_type: str
    ) -> Dict[str, Any]:
        """测试单个用例"""
...
@pytest.mark.manual
@pytest.mark.skip(reason="Requires a live Galaxy NLU service — run manually")
def test_nlu_v2_live_suite():
    """Wraps the manual NLU v2 evaluation suite for discoverability."""
    import asyncio
