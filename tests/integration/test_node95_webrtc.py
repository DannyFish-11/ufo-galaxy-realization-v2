# ---------------------------------------------------------------------------
# CANONICAL LOCATION: tests/integration/test_node95_webrtc.py
# Relocated from nodes/Node_95_WebRTC_Receiver/test_node95.py
#
# NOTE: This is a manual integration test that requires a live Node_95
# WebRTC receiver service on http://localhost:8095.  It is marked ``manual``
# so that it is skipped in standard CI runs.  Run explicitly with:
#   pytest tests/integration/test_node95_webrtc.py -m manual
# ---------------------------------------------------------------------------
"""
Node_95 测试脚本
...
try:
    ...
    pytest.skip(f"httpx/websockets not available: {_e}", allow_module_level=True)
...
async def test_http_api():
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(
            f"{NODE_95_URL}/get_latest_frame", json={"device_id": "test_device", "format": "jpeg"}
        )
        print(f"Status: {response.status_code}")
        print(f"Response: {json.dumps(response.json(), indent=2)}")
...
async def test_websocket_connection():
    ...
    try:
        async with websockets.connect(f"{NODE_95_WS}/signaling/{device_id}") as websocket:
            ...
            test_message = {"type": "ice_candidate", "candidate": "candidate:test", "sdpMid": "0", "sdpMLineIndex": 0}
...
"""
    ...
    try:
        async with websockets.connect(f"{NODE_95_WS}/signaling/{device_id}") as websocket:
            ...
            # 发送 Offer
            print("\n3.2 发送 Offer")
            offer_message = {"type": "offer", "sdp": mock_offer_sdp}
...
@pytest.mark.manual
@pytest.mark.skip(reason="Requires a live Node_95 WebRTC service on localhost:8095 — run manually")
def test_node95_webrtc_live_suite():
    """Wraps the manual Node_95 WebRTC integration test for discoverability."""
    import asyncio

    success = asyncio.run(run_all_tests())
    assert success, "Node_95 WebRTC integration tests failed"
