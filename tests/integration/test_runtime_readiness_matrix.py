...
class TestReadinessMatrixStructure:
    ...
    def test_matrix_evaluates_all_required_dimensions(self) -> None:
        ...
        assert not missing, f"Required matrix dimensions not evaluated: {missing}"
    ...
    def test_matrix_produces_valid_verdict(self) -> None:
        ...
        assert matrix.verdict.value in valid_verdicts, f"Matrix verdict {matrix.verdict!r} is not a valid MatrixVerdict"
...
class TestCriticalDimensionsPass:
    ...
    def test_transport_layer_dimension_passes(self) -> None:
        ...
        assert transport.status == DimensionStatus.PASSED, f"transport_layer dimension failed: {transport.detail}"
    ...
    def test_canonical_execution_path_passes(self) -> None:
        ...
        assert dim.status == DimensionStatus.PASSED, f"canonical_execution_path failed: {dim.detail}"
    ...
    def test_capability_enforcement_dimension_passes(self) -> None:
        ...
        assert dim.status == DimensionStatus.PASSED, f"capability_enforcement failed: {dim.detail}"
    ...
    def test_protocol_regression_surface_passes(self) -> None:
        ...
        assert dim.status == DimensionStatus.PASSED, f"protocol_regression_surface failed: {dim.detail}"
    ...
    def test_aip_v3_version_stability_passes(self) -> None:
        ...
        assert dim.status == DimensionStatus.PASSED, f"aip_v3_version_stability failed: {dim.detail}"
    ...
    def test_nats_posture_contract_passes(self) -> None:
        ...
        assert dim.status == DimensionStatus.PASSED, f"nats_posture_contract failed: {dim.detail}"
...
class TestReleaseBlockingGate:
    ...
    def test_gate_evaluates_all_criteria(self) -> None:
        ...
        assert len(decision.criteria) >= 4, f"Expected at least 4 gate criteria, got {len(decision.criteria)}"
    ...
    def test_run_release_blocking_gate_returns_zero(self) -> None:
        ...
        assert exit_code == 0, f"run_release_blocking_gate() returned {exit_code} (expected 0)"
