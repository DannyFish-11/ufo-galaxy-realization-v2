...
def test_key_nodes():
    ...
    for node_name, description in key_nodes:
        ...
        assert os.path.exists(node_path), f"{node_name} ({description}): 不存在"
        with open(node_path, "r", encoding="utf-8") as f:
            lines = len(f.readlines())
        assert lines > 50, f"{node_name} ({description}): {lines} 行代码 (可能是空壳)"
...
def test_ui_files():
    ...
    android_ui_layout = os.path.join(
        android_root, "app", "src", "main", "res", "layout", "floating_window_dynamic_island.xml"
    )
    android_ui_code = os.path.join(
        android_root, "app", "src", "main", "java", "com", "galaxy", "app", "ui", "DynamicIslandFloatingWindow.kt"
    )
...
def main():
    ...
    tests = [test_core_modules, test_key_nodes, test_ui_files, test_code_statistics]
