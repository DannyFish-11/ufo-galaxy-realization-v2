...
class TestFormationParticipantStatus:
    ...
    def test_ready_is_viable(self):
        ...
        s = FormationParticipantStatus(device_id="d", state=FormationParticipantState.READY)
    ...
    def test_degraded_is_viable(self):
        ...
        s = FormationParticipantStatus(device_id="d", state=FormationParticipantState.DEGRADED)
        assert s.is_viable is True
    ...
    def test_lost_is_not_viable(self):
        ...
        s = FormationParticipantStatus(device_id="d", state=FormationParticipantState.LOST)
    ...
    def test_recovering_is_not_viable(self):
        ...
        s = FormationParticipantStatus(device_id="d", state=FormationParticipantState.RECOVERING)
        assert s.is_viable is False
    ...
    def test_to_dict(self):
        ...
        s = FormationParticipantStatus(device_id="d", state=FormationParticipantState.READY, health_score=0.8)
        d = s.to_dict()
...
class TestOnParticipantReadinessChanged:
    def test_ready_state_no_rebalance(self):
        ...
        group = _make_group()
        coord = FormationRuntimeCoordinator(group)
        decision = coord.on_participant_readiness_changed("primary-dev", FormationParticipantState.READY)
...
class TestSourcePreservation:
    def test_source_not_targeted_by_terminate_action(self):
        ...
        decision = coord.on_participant_lost("primary-dev")
        terminate_targets = [
            a.affected_device_id for a in decision.recommended_actions if a.action_type == RecoveryActionType.TERMINATE
        ]
...
class TestMakeFormationRuntimeCoordinator:
    ...
    def test_factory_custom_thresholds(self):
        ...
        group = _make_group(support_ids=["sup-1"])
        coord = make_formation_runtime_coordinator(group, unhealthy_threshold=0.5, degraded_threshold=0.8)
...
class TestHarnessOnParticipantReadinessChanged:
    def test_smoke_no_formation(self):
        ...
        result = harness.on_participant_readiness_changed("dev-1", "degraded", health_score=0.4)
    ...
    def test_smoke_with_formation(self):
        ...
        harness = MultiDeviceRuntimeHarness()
        group = _make_group(support_ids=["sup-1"])
        result = harness.on_participant_readiness_changed("sup-1", "lost", formation=group, reason="timeout")
        assert result is not None
    ...
    def test_smoke_lost_triggers_rebalance_on_primary(self):
        ...
        harness = MultiDeviceRuntimeHarness()
        group = _make_group(fallback_ids=["fb-1"])
        result = harness.on_participant_readiness_changed("primary-dev", "lost", formation=group)
...
class TestModuleLevelOnParticipantReadinessChanged:
    ...
    def test_module_level_with_formation(self):
        ...
        reset_multi_device_runtime_harness()
        group = _make_group(fallback_ids=["fb-1"])
        result = on_participant_readiness_changed("primary-dev", "lost", formation=group)
        assert result.rebalance_triggered is True
