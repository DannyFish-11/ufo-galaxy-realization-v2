...
class TestAppThin:
    ...
    def test_app_py_line_count_reduced(self):
        ...
        assert count <= 250, f"galaxy_gateway/app.py has {count} lines; expected ≤ 250 after PR-2 slimming."
    ...
    def test_app_py_imports_lifespan_from_bootstrap(self):
        ...
        src = _source("galaxy_gateway/app.py")
        assert (
            "from galaxy_gateway.bootstrap" in src or "from .bootstrap" in src
    ...
    def test_app_py_imports_bearerauth_from_middleware(self):
        ...
        src = _source("galaxy_gateway/app.py")
        assert (
            "from galaxy_gateway.middleware import" in src or "from .middleware import" in src
...
class TestChatRequestInRouteModule:
    ...
    def test_chatrequest_defined_in_routes_chat(self):
        ...
        assert "class ChatRequest" in src, "ChatRequest class must be defined in galaxy_gateway/routes/chat.py"

    def test_multimodal_context_field_in_routes_chat(self):
        src = _source("galaxy_gateway/routes/chat.py")
        assert "multimodal_context" in src, "multimodal_context field must appear in galaxy_gateway/routes/chat.py"
...
class TestLifecycleModule:
    ...
    def test_lifecycle_source_initializes_device_manager(self):
        ...
        assert "DeviceManager()" in src, "lifecycle.py must instantiate DeviceManager during startup"

    def test_lifecycle_source_sets_app_state(self):
        src = _source("galaxy_gateway/bootstrap/lifecycle.py")
        assert "app.state" in src, "lifecycle.py must store services on app.state for dependency injection"

    def test_lifecycle_source_updates_module_globals(self):
        src = _source("galaxy_gateway/bootstrap/lifecycle.py")
        assert (
            "_gw_app.websocket_manager" in src or "galaxy_gateway.app" in src
