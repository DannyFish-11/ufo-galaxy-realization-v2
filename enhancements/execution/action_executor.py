...
class ActionExecutor:
    ...
    async def execute_action(self, action, context: ExecutionContext, world_model=None) -> ExecutionResult:
        ...
        try:
            ...
            return ExecutionResult(
                ...
                duration=duration,
                output=output,
                metadata={"command": action.command, "parameters": action.parameters},
            )
    ...
    async def _execute_command(self, action) -> Any:
        ...
        if command == "wait":
            ...
            # 未知命令必须**判失败**,不能返回一个看起来正常的 dict。
            #
            # 根因(实测):本执行器的直执路径只认识 wait / log 两个命令,而
            # AutonomousPlanner._generate_command() 生成的是 web_search /
            # read_file / write_file / execute_command / analyze_data /
            # synthesize_info / device_control / send_message —— **一个都不认识**。
            # 原实现在这里返回 {"status": "unknown"} 且不抛异常,于是外层
            # execute_action() 无条件把它包成 ExecutionStatus.SUCCESS:
            # 每个动作都"成功"、成功率 100%、而实际上什么都没做。
            # 这些假成功还会流进监控与学习层,把"没干成"当成经验学下去。
            #
            # 这里改为抛异常,让 execute_action() 的既有 except 分支如实记为
            # 失败。要真正执行这些命令,应当接 core.capabilities.CanonicalDispatcher
            # (系统唯一被主链验证过的执行底座),而不是在此扩充命令表。
            logger.warning("未知命令(判失败,不伪装成功): %s", command)
            raise ValueError(
                f"ActionExecutor 不支持命令 {command!r}(仅支持 wait/log);请改用 CanonicalDispatcher 执行真实动作"
            )
