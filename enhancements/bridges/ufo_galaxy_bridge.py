...
2. 实现 ufo-galaxy 与微软 Galaxy 之间的双向互调
3. 自动检测两个系统的存在并建立连接
...
class GalaxyGalaxyBridge:
    """
    零破坏性桥接器
    
    作为独立的"外骨骼"，连接 ufo-galaxy 和微软 Galaxy
    """
    ...
    async def initialize(self):
        ...
        if not self.ufo_galaxy_available and not self.microsoft_ufo_available:
            ...
            available = "ufo-galaxy" if self.ufo_galaxy_available else "微软 Galaxy"
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=payload) as resp:
                    result = await resp.json()
                    logger.info(f"✅ 成功调用微软 Galaxy {agent_name}")
                    return result
        except Exception as e:
            logger.error(f"❌ 调用微软 Galaxy 失败: {e}")
            return {"error": str(e)}
    ...
    async def unified_vision_analysis(self, image_path: str, query: str) -> Dict[str, Any]:
        ...
        优先使用 ufo-galaxy 的 Node_90，回退到微软 Galaxy（如果可用）
