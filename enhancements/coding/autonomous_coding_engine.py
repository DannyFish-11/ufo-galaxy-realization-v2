...
class AutonomousCodingEngine:
    ...
    def __init__(self, workspace_root: str = None):
        ...
        self._exec_log_handler.setFormatter(logging.Formatter("%(asctime)s %(levelname)s %(message)s"))
    ...
    def _check_command(self, command: str) -> bool:
        """检查命令是否可用"""
        try:
            subprocess.run([command, "--version"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=5)
            return True
    ...
    async def _generate_code(self, context: CodingContext, analysis: Dict, tools: List[str]) -> Dict[str, str]:
        """
        生成代码
            ...
            data = json.dumps(
                {
                    ...
                    "messages": [
                        {
                            "role": "system",
                            "content": "你是一个 Python 代码生成器。只输出 Python 代码，不要 markdown 包裹。",
                        },
...
'''
        ...
        if "pytest" in self.available_tools:
            for file_path in files:
                if "_test.py" in file_path or "test_" in file_path:
                    ...
                    try:
                        # async 路径里同步跑 pytest 会冻事件循环最长 30s,放线程
                        ...
                        test_results["details"].append({"file": file_path, "output": output})
    ...
    def _record_coding_history(
        self, context: CodingContext, modified_files: List[str], created_files: List[str], quality: CodeQuality
    ):
        """记录编程历史"""
        record = {
