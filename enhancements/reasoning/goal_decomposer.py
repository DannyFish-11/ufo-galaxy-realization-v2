...
class GoalDecomposer:
    ...
    def decompose(self, goal: Goal) -> DecompositionResult:
        ...
        return DecompositionResult(
            goal=goal, subtasks=subtasks, execution_order=execution_order, estimated_total_duration=total_duration
        )
    ...
    def _generate_subtasks_with_llm(self, goal: Goal, goal_type: GoalType) -> List[SubTask]:
        ...
        if asyncio.iscoroutinefunction(getattr(self.llm_client, "chat", None)):
            # 在同步上下文中调用异步方法。
            # 修复:原来写 `loop = asyncio.get_running_loop(); if loop.is_running(): ... else: asyncio.run(...)`
            # ——get_running_loop() 在【没有】运行循环时直接抛 RuntimeError,根本
            # 走不到 else 分支:纯同步上下文(如 CLI/线程池)调用必崩。正确写法
            # 是用异常判定"有没有运行中的循环"。
            try:
                ...
                # 有运行循环(被误从事件循环线程同步调用)→ 换线程跑,避免死锁
                import concurrent.futures

                with concurrent.futures.ThreadPoolExecutor() as pool:
                    resp = pool.submit(
                        asyncio.run, self.llm_client.chat(messages=messages, task_type="planning")
                    ).result()
        else:
