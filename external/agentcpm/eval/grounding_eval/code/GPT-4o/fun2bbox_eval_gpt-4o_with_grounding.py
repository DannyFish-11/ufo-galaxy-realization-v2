...
async def call_GPT(item, client, model_name, new_image_dir):
    """
    调用Qwen输出function的描述，输出bbox
    """
    system_prompt = '''
    你是一个GUI组件定位的专家，擅长根据组件的功能描述输出对应的位置。你的任务是根据给定的GUI截图和图中某个组件的功能描述输出与组件最接近的框的编号
    输入：屏幕截图，文本描述
    ...
    new_image_path = item["image"].replace(old_image_dir, new_image_dir)
    ...
    content.append({
        "type": "text",
        "text": "屏幕上组件的功能描述为：{}".format(item["text"])
    })
...
async def verify(response, json_list, ground_truth,w, h):
    """
    接受模型的字符串输入，判断是否正确
    """
    pattern = r'<id>\d+</id>'
    ...
    if matches:
        ...
        gt_y_min = gt_bbox[1]
        gt_y_max = gt_bbox[3]
        pre_x_min = pre_bbox[0]*w
        pre_x_max = pre_bbox[2]*w
        pre_y_min = pre_bbox[1]*h
        pre_y_max = pre_bbox[3]*h
...
async def process_item_async(item, json_item, client, model_name, semaphore, new_image_dir):
    async with semaphore:
        ...
        w = image.width
        h = image.height
        bbox = await call_GPT(item, client, model_name, new_image_dir)
        correct = await verify(bbox,json_item ,item["abs_position"],w,h)
        return correct
