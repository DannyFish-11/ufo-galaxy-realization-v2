...
async def call_Qwenvl(item, client, model_name):
    ...
    输入：屏幕截图，边界框的相对坐标,缩放至0~1000, Action:click(start_box='(x_min,y_min,x_max,y_max)')的格式表示
    输出：组件中的文本,注意是文字而非坐标！'''
    image_path = item["image"]
    base64_image, w, h = encode_image(image_path)
    bbox = process_position(item, w, h)
    content = []
    ...
    content.append({
        "type": "text",
        "text": "屏幕上某一组件的边界框：Action:click(start_box='({},{},{},{})')".format(bbox[0],bbox[1],bbox[2],bbox[3])
    })
...
async def main():
    ...
    client = AsyncClient(api_key=os.getenv("OPENAI_API_KEY", ""), base_url='http://localhost:8000/v1')
