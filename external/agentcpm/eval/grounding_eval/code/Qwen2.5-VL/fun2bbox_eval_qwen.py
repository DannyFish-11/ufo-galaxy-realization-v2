...
async def call_Qwenvl(item, client, model_name):
    ...
    输入：屏幕截图，功能描述
    输出：边界框的绝对坐标，以<x,y>为格式，使用<>定位，其中不能存在任何非坐标字符
    示例输出：我认为该UI元素在<600,1000>附近
    '''
...
async def main():
    ...
    client = AsyncClient(api_key=os.getenv("OPENAI_API_KEY", ""), base_url='http://localhost:8001/v1')
