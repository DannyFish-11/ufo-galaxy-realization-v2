...
// 端口优先级: 环境变量 GALAXY_ELECTRON_PORT > GATEWAY_PORT > 默认 9229
const IPC_HTTP_PORT = parseInt(
    process.env.GALAXY_ELECTRON_PORT ||
    process.env.GATEWAY_PORT ||
    '9229',
    10
);
// Python 后端 API 基础 URL（Settings 配置保存用）
const PY_BACKEND_URL = `http://127.0.0.1:${IPC_HTTP_PORT}`;
...
async function fetchConfigFromBackend() {
    try {
        const response = await fetch(`${PY_BACKEND_URL}/api/config`);
        if (response.ok) {
            configCache = await response.json();
            return configCache;
        }
    } catch (e) {
        console.error('[Main] Failed to fetch config from', PY_BACKEND_URL, ':', e.message);
    }
    return configCache;
}
...
ipcMain.handle('galaxy:save-config', async () => {
    try {
        const response = await fetch(`${PY_BACKEND_URL}/api/config/save`, {
            method: 'POST',
        });
