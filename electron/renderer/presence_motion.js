 ...
 * 刻意保留的东西
 * --------------
 * **编排限速器不删**。理由与渲染方式无关，所以换掉 WebGL 之后原样成立：
 * 编排是**分段**的（现在是 app.js 的 SEG：边光收回 → 四壁延伸 → 灵动岛长出），
 * 每一段都假设展开度匀速穿越；而相位事件仍是离散跳变（广播是事件驱动的，
 * 不是每帧一拍）。纯弹簧约 100ms 就冲过整个收回窗口，那几段动作在数学上
 * 就不可能被看见。这里改的是**速度怎么定**，不是要不要限速。
 *
