...
ENV_TEMPLATE = """# Galaxy Configuration
...
def generate_env_file(path: str, force: bool = False):
    ...
    if os.path.exists(path) and not force:
        print(f"⚠️  .env file already exists at {path}")
...
WHITELIST_TEMPLATE = {
    "version": "1.0",
    "description": "Galaxy Security Whitelist",
    "rules": [
...
def generate_whitelist(path: str, force: bool = False):
    ...
    if os.path.exists(path) and not force:
        print(f"⚠️  whitelist.json already exists at {path}")
        return False
    
    with open(path, "w") as f:
        json.dump(WHITELIST_TEMPLATE, f, indent=2)
    
    print(f"✅ Generated whitelist.json at {path}")
    return True
...
class GalaxyLauncher:
    ...
    def check_prerequisites(self) -> bool:
        ...
        if not os.path.exists(env_file):
            print(f"⚠️  .env file not found, generating...")
        ...
        if not os.path.exists(whitelist_file):
            print(f"⚠️  whitelist.json not found, generating...")
        ...
        try:
            ...
            print(f"❌ Container runtime not available: {e}")
            return False
        ...
        for node in nodes:
            try:
                ...
                if result.returncode != 0:
                    ...
                    if critical:
                        print(f"❌ Critical node {node} failed to start")
                        return False
                else:
    ...
    def start(self, skip_build: bool = False) -> bool:
        """Start the Galaxy system."""
        print("""
╔═══════════════════════════════════════════════════════════╗
║         Galaxy 64-Core MCP Matrix                     ║
║         Launcher v2.0 - DeepSeek Audited                  ║
╚═══════════════════════════════════════════════════════════╝
        """)
        ...
        for i, phase in enumerate(STARTUP_PHASES):
            ...
            if not self.start_phase(phase):
                print(f"\n❌ Startup failed at phase: {phase['name']}")
                return False
        
        print("\n" + "="*60)
        print("🎉 Galaxy started successfully!")
        print("="*60)
...
def main():
    parser = argparse.ArgumentParser(
        description="Galaxy Launcher v2.0",
