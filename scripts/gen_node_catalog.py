...
#
# 【补全】此前这张表只覆盖了各节点的一部分动作。那时无所谓 —— HTTP 路由根本不过闸,
# "表里没有"不等于"做不到"。现在 HTTP 面也要过闸,表若仍然残缺,后果就从"漏防"
# 变成"把能用的路由 403 打死"。所以先按各节点**真实路由**把表补齐:
# manifest 得先诚实地描述节点能做什么,收紧才有意义 —— 从一张残缺的表上删条目,
# 删掉的是本来就没生效的东西。
...
_ACTION_PERMISSIONS: dict = {
    ...
    # ShellExecutor —— 动作名核实自它的真实路由。此前这一条只写在生成产物
    # config/node_catalog.json 里,而那个文件头一行就是"由脚本生成,勿手改":
    # 下次 regenerate 就会被抹掉,权限声明连同它挡住的东西一起消失,且没有任何现象。
    122: [
        "execute",
        "script",
        "background",
