...
class ResumableTransferManager:
    ...
    def create_session(self, session_id: str, file_path: str, chunk_size: int = None) -> TransferSession:
        """
        创建传输会话
        ...
        while offset < file_size:
            ...
            chunks.append(ChunkInfo(index=index, offset=offset, size=size))
            offset += size
            index += 1
        ...
        try:
            ...
            for chunk in session.chunks:
                ...
                if progress_callback and time_delta >= self.config.PROGRESS_INTERVAL:
                    ...
                    speed = (
                        ((session.transferred_bytes - last_transferred_bytes) / time_delta) if time_delta > 0 else 0.0
                    )
    ...
    async def write_chunk(self, session_id: str, chunk_index: int, chunk_data: bytes) -> bool:
        """
        写入分块数据
...
async def example_usage():
    ...
    success = await manager.send_file(session_id, mock_send_chunk, progress_callback)
