...
class MessageBuilder:
    ...
    def create_file_message(
        ...
        payload = {
            ...
            "transfer_method": (TransferMethod.GATEWAY.value if file_size < 1024 * 1024 else TransferMethod.P2P.value),
            "chunks": chunks,
            "from_device": from_device.device_id,
            "to_device": to_device.device_id,
            **(metadata or {}),
...
class MultimodalTransferManager:
    ...
    async def send_image(
        ...
        # 压缩图片（如果需要）
        if compress:
            image_data = await self._compress_image(image_data, quality=quality or self.config.IMAGE_QUALITY)
        ...
        message = MessageBuilder.create_image_message(
            ...
            format=format,
            metadata={"width": width, "height": height, "filename": filename, "compressed": compress},
        )
    ...
    async def send_video(
        self, from_device: DeviceInfo, to_device: DeviceInfo, video_path: str, metadata: Dict[str, Any] = None
    ) -> AIPMessage:
        """
        发送视频
    ...
    async def send_audio(
        ...
        size = len(audio_data)
        transfer_method = (
            TransferMethod.GATEWAY.value if size < self.config.GATEWAY_MAX_SIZE else TransferMethod.P2P.value
        )
        payload_dict = {
            "data_type": "audio",
            "format": format,
    ...
    async def send_file(
        self, from_device: DeviceInfo, to_device: DeviceInfo, file_path: str, metadata: Dict[str, Any] = None
    ) -> AIPMessage:
        """
        发送文件
    ...
    async def send_file_chunk(self, file_path: str, chunk_index: int, chunk_size: int) -> bytes:
        """
        发送文件分块
    ...
    async def receive_file_chunk(self, file_path: str, chunk_index: int, chunk_data: bytes):
        """
        接收文件分块
    ...
    async def send_screenshot(
        self, from_device: DeviceInfo, to_device: DeviceInfo, screenshot_data: bytes, metadata: Dict[str, Any] = None
    ) -> AIPMessage:
        """
        发送屏幕截图
...
async def example_usage():
    ...
    image_msg = await manager.send_image(from_device=phone_a, to_device=pc, image_data=image_data, compress=False)
    ...
    audio_msg = await manager.send_audio(
        from_device=phone_a, to_device=pc, audio_data=fake_audio_data, format="mp3", metadata={"duration": 30}
    )
    ...
    file_msg = await manager.send_file(
        from_device=phone_a, to_device=pc, file_path=temp_file, metadata={"description": "Large binary file"}
    )
