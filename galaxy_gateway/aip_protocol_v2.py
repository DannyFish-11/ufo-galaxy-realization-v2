...
class ExtendedMessageType(Enum):
    ...
    # ========== Agent 分发 (4个) — Phase 1 Matrix OS ==========
    AGENT_DEPLOY = "agent_deploy"                 # 部署 Agent 到端侧 (携带 AgentManifest)
    AGENT_DEPLOY_ACK = "agent_deploy_ack"         # 端侧确认接收
    AGENT_STATUS = "agent_status"                 # Agent 执行状态上报 (Thought/Action/Observation)
    AGENT_RESULT = "agent_result"                 # Agent 执行完成结果

    # ========== 设备间中继 (4个) — Phase 2 Proxy-Relay ==========
    RELAY_REQUEST = "relay_request"               # 设备 A 发起转发请求
    RELAY_FORWARD = "relay_forward"               # 服务端转发给设备 B
    RELAY_REPLY = "relay_reply"                   # 设备 B 返回响应
    RELAY_ACK = "relay_ack"                       # 服务端确认转发成功

    # ========== 混合执行 (3个) — Phase 3 Hybrid Execution ==========
    HYBRID_EXECUTE = "hybrid_execute"             # 混合执行请求
    HYBRID_RESULT = "hybrid_result"               # 混合执行结果
    HYBRID_DEGRADE = "hybrid_degrade"             # 降级通知 (A2A→GUI→VLM)

    # ========== RAG & 代码执行 (3个) — Phase 4 ==========
    RAG_QUERY = "rag_query"                       # RAG 知识检索请求
    RAG_RESULT = "rag_result"                     # RAG 检索结果
    CODE_EXECUTE = "code_execute"                 # 安全代码执行请求

    # ========== P2P Mesh Overlay (3个) — Phase 5 ==========
    PEER_ANNOUNCE = "peer_announce"               # 设备上报 LAN 信息
    PEER_EXCHANGE = "peer_exchange"               # 服务端下发 peer 列表
    MESH_TOPOLOGY = "mesh_topology"               # 拓扑同步
