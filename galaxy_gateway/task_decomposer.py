...
class TaskDecomposer:
    ...
    def decompose_file_transfer(
        self, source_device_id: str, target_device_id: str, file_path: str, task_id_prefix: str = "task"
        ...
        Returns:
            (任务列表, 数据流列表)
        """
        from enhanced_nlu_v2 import IntentType, Task
        ...
        data_flow = DataFlow(
            from_task_id=task_1.task_id, to_task_id=task_2.task_id, data_key="file_content", transform=None
        )
    ...
    def decompose_send_and_open(
        ...
        task_3 = Task(
            ...
            target=app_to_open,
            parameters={"file_path": file_path, "app": app_to_open},
    ...
    def decompose_multi_device_command(
        self, commands: List[Dict[str, Any]], task_id_prefix: str = "task"
    ) -> tuple[List[Any], List[DataFlow]]:
        """
        分解多设备命令
    ...
    def decompose_search_and_open(
        self, device_id: str, browser: str, search_keyword: str, task_id_prefix: str = "task"
    ) -> tuple[List[Any], List[DataFlow]]:
        """
        分解"搜索并打开"任务
    ...
    def decompose_conditional_task(
        self, condition_task: Any, true_tasks: List[Any], false_tasks: List[Any], task_id_prefix: str = "task"
    ) -> tuple[List[Any], List[DataFlow]]:
        """
        分解条件任务
...
class IntelligentTaskPlanner:
    ...
    async def plan_complex_task(self, user_input: str, context: Any = None) -> tuple[List[Any], List[DataFlow]]:
        """
        规划复杂任务
...
async def main():
    ...
    tasks, flows = decomposer.decompose_file_transfer(
        source_device_id="phone_a", target_device_id="pc", file_path="/sdcard/photo.jpg"
    )
    ...
    tasks, flows = decomposer.decompose_send_and_open(
        source_device_id="phone_a", target_device_id="pc", file_path="/sdcard/photo.jpg", app_to_open="photoshop"
    )
    print(f"任务数: {len(tasks)}")
    print(f"数据流: {len(flows)}")
    for task in tasks:
    ...
    tasks, flows = await planner.plan_complex_task("把手机 A 上的照片发到电脑，然后用 PS 打开")
    print(f"任务数: {len(tasks)}")
    print(f"数据流: {len(flows)}")
    for task in tasks:
        print(f"  - {task.task_id}: {task.action} on {task.device_id}")
