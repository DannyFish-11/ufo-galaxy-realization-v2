...
class WebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, device_id: str) -> bool:
        ...
        try:
            ...
            self.connections[device_id] = DeviceConnection(device_id=device_id, connected_at=now, last_heartbeat=now)
    ...
    async def disconnect(self, device_id: str):
        ...
        if now_ts - prev_backoff < 120:  # double backoff if disconnect within 2min
            self._reconnect_backoff[device_id] = now_ts + min(
                self._backoff_max,
                self._backoff_base * 2 ** len([k for k, v in self._reconnect_backoff.items() if v > now_ts]),
            )
        else:
    ...
    async def _handle_heartbeat(self, device_id: str, message: AIPMessage):
        """处理心跳消息"""
        ack = AIPMessage(type=MessageType.DEVICE_HEARTBEAT_ACK, device_id=device_id, correlation_id=message.message_id)
