...
class TaskScheduler:
    ...
    def create_execution_plan(self, tasks: List[Any]) -> ExecutionPlan:
        ...
        estimated_duration = sum(
            max(task_map[task_id].estimated_duration for task_id in layer) for layer in execution_order
        )
...
class TaskRouter:
    ...
    async def execute_tasks(self, tasks: List[Any], trace_id: Optional[str] = None) -> List[TaskResult]:
        ...
        try:
            from galaxy_gateway.autonomous_filter import _prefer_autonomous, is_autonomous_device
        ...
        for layer_index, layer in enumerate(plan.execution_order):
            ...
            layer_results = await asyncio.gather(
                *[self._execute_single_task(task, trace_id=_trace_id) for task in layer_tasks], return_exceptions=True
            )
    ...
    async def _send_task_to_device(self, device, task_message: Dict[str, Any]) -> Any:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=task_message, timeout=aiohttp.ClientTimeout(total=30)) as response:
...
class ResultAggregator:
    ...
    def aggregate(self, results: List[TaskResult]) -> Dict[str, Any]:
        ...
        # 收集错误
        errors = [
            {"task_id": r.task_id, "device_id": r.device_id, "error": r.error}
            for r in results
