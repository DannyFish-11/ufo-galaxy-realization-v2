...
class STUNClient:
    ...
    @staticmethod
    def _blocking_stun_request(local_port: int, stun_server: Tuple[str, int]) -> Tuple[Optional[str], Optional[int]]:
    ...
    async def get_public_address(
        local_port: int, stun_server: Tuple[str, int] = None
        ...
        return await asyncio.to_thread(STUNClient._blocking_stun_request, local_port, stun_server)
...
class P2PConnector:
    ...
    async def start(self):
        """启动 P2P 连接器"""
        # 发现公网地址
        public_ip, public_port = await STUNClient.get_public_address(self.local_device.local_port)
    ...
    async def connect(self, peer: PeerInfo) -> bool:
        ...
        try:
            ...
            if peer.local_ip:
                try:
                    reader, writer = await asyncio.wait_for(
                        asyncio.open_connection(peer.local_ip, peer.local_port), timeout=P2PConfig.CONNECTION_TIMEOUT
                    )
                    conn.reader = reader
                    conn.writer = writer
                    connected = True
            ...
            if not connected and peer.public_ip:
                try:
                    reader, writer = await asyncio.wait_for(
                        asyncio.open_connection(peer.public_ip, peer.public_port), timeout=P2PConfig.CONNECTION_TIMEOUT
                    )
                    conn.reader = reader
                    conn.writer = writer
                    connected = True
    ...
    async def _run_server(self):
        """运行服务器（监听连接）"""
        try:
            server = await asyncio.start_server(self._handle_client, "127.0.0.1", self.local_device.local_port)
    ...
    async def _receive_loop(self, conn: P2PConnection):
        ...
        try:
            while conn.state == ConnectionState.CONNECTED:
                # 读取数据长度（带超时，防止永久挂起）
                length_data = await asyncio.wait_for(conn.reader.readexactly(4), timeout=self.READ_TIMEOUT)
                length = struct.unpack("!I", length_data)[0]
                ...
                # 读取数据（带超时，防止永久挂起）
                data = await asyncio.wait_for(conn.reader.readexactly(length), timeout=self.READ_TIMEOUT)
...
async def example_usage():
    ...
    # 创建两个设备
    device_a = PeerInfo(device_id="phone_a", device_name="手机A", local_ip="127.0.0.1", local_port=9001)

    device_b = PeerInfo(device_id="phone_b", device_name="手机B", local_ip="127.0.0.1", local_port=9002)
