...
### 1.4 PR 编号核验（已校验）
- V2：`#1102`、`#1103`（均已合并）
- Android：`#383`、`#384`（均已合并）
- 核验方式：GitHub API `list_pull_requests` + `pull_request_read(get/get_files)`。

### 1.5 PR 可复核元数据（节选）
| 仓库 | PR | merged_at (UTC) | head SHA（节选） |
|---|---|---|---|
| V2 | #1102 | 2026-05-11T04:12:29Z | `a15c481b...` |
| V2 | #1103 | 2026-05-11T04:12:49Z | `9828260f...` |
| Android | #383 | 2026-05-10T22:24:26Z | `6f8216f3...` |
| Android | #384 | 2026-05-11T03:32:02Z | `1600ab0f...` |
...
| 维度 | a 主链(40) | b 异常(25) | c 自动化(20) | d 实机长期(15) | 合计 |
|---|---|---|---|---|---|
| 双仓整体 | 29 | 16 | 13 | 7 | **65/100** |
| 本地链路 | 34 | 19 | 15 | 10 | **78/100** |
| 跨设备链路 | 24 | 13 | 12 | 9 | **58/100** |
| 系统语义成立度 | 27 | 15 | 12 | 8 | **62/100** |

- 分值依据：来自本文件第 2～4 节的已合并代码证据与测试证据（尤其 V2 `#1102/#1103` 与 Android `#383`）。
- 计算说明：四列上限分别为 40/25/20/15，总分上限 100；“合计”即四列加总后换算的完成度百分比。

### 5.2 评分标尺与不确定性口径
- 单项打分步长：按 1 分粒度给分（避免 5 分阶梯过粗）。
- `a 主链`：看是否存在 canonical 主链 + 是否可代码追踪到闭合（如 `task_result_canonical_truth_chain`、`query_closed_loop_governance_state`）。
- `b 异常`：看 delayed/conflicting/partial/recovered/timeout 是否有明确判定分支与拒绝乐观闭环逻辑。
- `c 自动化`：看是否有跨仓/集成测试与回归钩子（如 `test_pr13a...`、`Pr11B...HooksTest`）覆盖核心流。
- `d 实机长期`：看是否有持续性实机证据与长期门禁；不足时该项低分。
- ±区间规则：
  - 本地链路（±7%）：代码闭合与自动化证据较充分；
  - 整体/语义（±8%）：跨仓证据较多但仍缺长期实机门禁；
  - 跨设备（±10%）：复杂故障与长期实机证据最弱，不确定性最大；
  - 因此并非任意给区间，而是按证据完整度分层。
