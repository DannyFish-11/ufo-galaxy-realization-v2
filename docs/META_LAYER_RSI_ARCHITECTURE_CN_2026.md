...
### 3.2bis 三层切分（已作废，见 §8.1）

> **作废**：下表把三态划给常驻的 `DeviceSubject`。所有者已澄清三态属于电脑
> （desktop runtime = 后端 ＋ Electron 前端），手机与手表不走三态。
> 以 §8.1 为准，本表仅作走错路的记录保留。
...
> 本节记录三处与前文冲突的修正。**冲突之处以本节为准**，前文保留不删——
> 本仓的惯例是把走错的路留在记录里，而不是抹掉。

## 8.1 修正一：三态属于电脑，不属于智能体

本节此前写过两个都不对的版本，一并作废：

1. §3.2bis 把三态划给常驻的 `DeviceSubject` —— **错**。
2. 本节初稿改写成「三态是一份进入协议，任何载体上的显现都走它，电脑走一份、
   手机走一份」—— **也错**，而且错得更隐蔽。

所有者的原话：*"你知道为什么电脑上是 desktop runtime 吗　包括这整套三态
其实就是电脑上的后端和前端"*、*"不是智能体跟着这三态走，是智能体在电脑上进入这三态"*、
*"总不能跨设备、多设备，什么都得经历一遍三态吧"*。

**正确的理解：三态就是 desktop runtime 本身——电脑上的后端 ＋ 前端。**

- 后端 = `core/desktop_presence_runtime.py`
- 前端 = Electron 覆盖层的三态视觉（SILENT 呼吸灯 / LIMINAL 白色脉冲 /
  MANIFEST 扫描线显形）

这一对配起来才叫「桌面运行时」，**它是电脑这台机器的属性**。智能体是独立的、
长时运行的；只有当它要在这台电脑上动手时，才进入这套三态。

**手机和手表不走三态。** 它们是有界相对主体，各自持有形状不同的本地运行时
（`contracts/distributed_subject_contract_v1.py` 已定义）。跨设备工作经 NATS / mesh
协调，全程不碰电脑的三态。
...
元层            —— 管「走哪条线 / 进不进 / 进多深」
  ↓
智能体本体      —— 一直在运行，不绑死在任何一台设备上
  ├─ 纯云端线    —— 不碰任何设备，不走三态
  ├─ 跨设备线    —— NATS / mesh，不走三态
  └─ 本地显现线  —— 进入电脑的 desktop runtime（= 三态）→ computer_use
```

**这解释了所有者为什么要在三态之上加元层**：元层管的正是「进入」这件事本身，
而不是进入之后的具体动作。同时它也回答了 §2.5 悬而未决的一点——元层如何
既不进热路径又能起作用：**它根本不在载体上，它在本体那条线上。**

## 8.2 修正二：不是打分，是证据——而且本仓已有这套东西 ★

前文（§1.5、§2.4、§7.2）反复使用「打分器」「scorer」「分数」。
**这个词用错了，而且错在方向上。**

所有者的判据：*"我不希望是打分制的"*、*"模型给自己打分是更加的不靠谱"*、
*"必须是基于它那个工具本身，或者它这个东西的东西，才能提供出来的"*。

区别在于**分数是被「生成」的，证据是被「观察」的**：

| 不要（分数） | 要（证据） |
|---|---|
| "这次修复质量 0.87" | `pytest` 退出码 = 1，失败的是 `test_foo.py::test_bar` |
| "我有 90% 把握点对了按钮" | 点完之后那张截图里，窗口标题变成了 X |
| "文件应该写成功了" | `stat` 那个文件：大小 4096，mtime 是刚才 |
| LLM 说「验证通过」 | `pytest -q` 的 stdout 原文 + 退出码 |

分数可以被优化、被放宽；证据是执行的**副产品**，不是执行的**目标**，所以无法被
朝着它优化。这也正是所有者提供的全部外部材料的共同点——重读 §7.2 那张表，
LongHorizon 的 Auditor *"校验真实环境输出"*、RSIAgent 的 Verifier *"把「我以为
成功」与环境真实状态分开"*、SoL-Pi 的归约器 *"逐条与归档原始日志比对"*、
Comp AI CRM *"只认客观物理证据"*——**没有一个是打分**。

### 本仓已经有证据模型，而且做得对

`core/execution_evidence_model.py`（1100 行）：

- **13 个证据状态**：`planned_not_started` / `started` / `locally_executed` /
  `android_delegated` / `partially_completed` / `completed_strong` /
  `completed_degraded` / `stale_result` / `duplicate_result` /
  `superseded_result` / `failed` / `aborted` / `interrupted`
- **4 个信任级别**：`trusted` / `provisional` / `quarantine` / `rejected`
- `classify_execution_evidence()` —— 模块自述 **"the canonical enforcement function"**

判据是**按优先级排序的确定性映射**，输入全是事实（是否重复 / 是否过期 /
是否被取代 / 真相链是否完整 / proof_class 是什么），**没有任何一处是分数**。

尤其注意 `provisional` 这一级——「有证据但未完全确认」。这正是 §2.1 要求的
「INCONCLUSIVE 是一等公民：没测出来不等于通过」。**本仓已经实现了这条。**

### 于是那个洞的真正形状比 §1.3 描述的更难堪

```
同一个系统里，两套完全相反的标准：

  一个任务执行完了算不算数   →  1100 行的证据模型 + 13 状态 + 4 信任级 + 执法函数
  一个代码补丁修好了没有     →  一个 LLM 自己填的布尔值，默认 true
```

**修法因此也变了：不是「发明一个裁决面」，是让 engineer 循环也走已有的证据模型。**

1. `engineer__apply` 之后，harness **自己**跑验证命令
2. 退出码 + stdout/stderr 原文存成证据（`core/context_archive.py` 已有归档能力）
3. 走 `classify_execution_evidence()` 定级
4. 只有 `trusted` 才写知识库；`provisional` 留证不进学习信号；
   `quarantine` / `rejected` 不算数

连新概念都不用引入。**M0/M1 的范围据此改写。**

> **一处必须分清的边界**：`core/continuum/decision_gate.py` 算的
> `value − interruption_cost − risk_cost → observe/hint/assist/execute`
> **是「决策」不是「裁决」**。决策（要不要做、做多狠）用确定性计算是对的；
> 裁决（做完了算不算数）绝不能用分数。两者不可混为一谈，本节只约束后者。

## 8.3 修正三：本地线与云端线必须并存——这是架构主干，不是优化项

所有者：*"既有一种本地的能力，同时又有云端运行的独立能力 …… 很明确，就是需要并存的，
包括模型供给上都是这样子的"*。
...
| | 模块 | 何时起 |
|---|---|---|
| **跨设备执行** | `core/cross_device_execution_chain.py`：OpenClawd → CommandRouter → TaskEnvelope → Gateway → Worker/Device | NATS 只在 `desktop-cross-device` 模式下启用（`core/system_mode.py:327`），**默认模式是 `DESKTOP_LOCAL`**（`:262`） |
| **本地执行** | `core/local_execution_chain.py`：OpenClawd → CommandRouter[LOCAL] → 本地执行器 → `LocalExecutionResult`；真正点击按键的是 `core/computer_use_loop.py` | 任务驱动，**有界**：步数上限 `GALAXY_CU_MAX_STEPS` 默认 15，另有 done/fail 与循环检测 |

**所有者的两点判断都与代码一致**：跨设备才起 NATS/mesh；本地执行是间歇的点击按键，
不是常驻循环（"不可能让电脑无时无刻都在点击"——`computer_use_loop` 15 步封顶）。

### 但这里有一个结构性问题

**现在所有路径都要经过 `DesktopPresenceRuntime.handle_request()`，也就是都要走一遍三态**
——包括那些根本不需要碰本机鼠标键盘的任务：查资料、整理记忆、跨设备协调、
元层自己的改进循环。

结合 §1.6 的更正（`RuntimeSession` 每请求创建即丢、`SILENT` 不在 tick 循环条件里），
后果是：**「智能体自己在跑」这件事，在架构上不存在。**
...
智能体本体（一直在跑）
  │
  ├─ 云端/后台线 ——— 不需要进入任何载体
  │                  查资料、整理记忆、跨设备协调、元层的 RSI 循环
  │                  不占鼠标键盘，可以持续运行
  │
  └─ 本地显现线 ——— 需要进入电脑
                     走进入协议（三态）：SILENT → LIMINAL → MANIFEST → SILENT
                     computer_use 点击按键，15 步封顶，做完退回 SILENT
                     ↓ 涉及其他设备时
                     经 NATS / mesh 协调（此时才起）
```
...
- `config/llm_routing_policy.yaml` —— LOCAL-FIRST，每个任务 priorities 以 ollama 打头，
  本地没跑自动回退云端
- `core/modality_capability.py` —— 三维协商（模型声明 × 服务现实 × **设备**）
- `core/provider_modality.py` —— 云端侧模态声明，修的正是「本地无视觉 + 有云端视觉 key
  却整场不被使用」这个误判
...
| PR | 内容 | 相对原版的变化 |
|----|------|---------------|
| **M0** | 守卫：扫「提案者自报成绩」缺陷签名 | 验收条件改为——守卫要同时指出 engineer 循环**没有走** `classify_execution_evidence()` |
| **M1** | engineer 循环接入已有证据模型：实跑 → 存原文证据 → `classify_execution_evidence()` 定级 → 只有 `trusted` 进知识库 | **从「发明裁决面」改为「复用已有证据模型」**，不引入新概念、不引入任何分数 |
| **M2** | Loop Kernel 骨架 | `Verdict` 的类型改为 `EvidenceTrustLevel`，不是分数 |
| **M3** | System Prompt 资产化 | 不变 |
| **M4** | 轨迹 → 用例 + 带前置条件的操作性假设 | 判定同样走证据，不走打分 |
| **M5** | **两条线分离**：云端/后台线不经 `handle_request()`；本地显现线保留三态 | **从「在场独立时钟」改为「本体线与载体线分离」**——时钟是表象，分线才是实质 |
| **M6** | 横轴 Curriculum | 跑在云端/后台线上 |
| **M7** | 表示层载荷槽位（仅记意图） | 不变 |
