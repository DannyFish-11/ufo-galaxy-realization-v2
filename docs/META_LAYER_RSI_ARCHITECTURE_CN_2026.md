...
## 8.1 修正一：三态是「进入协议」，不是设备的常驻状态

§3.2bis 把三态划给常驻的 `DeviceSubject`，**这是错的**。

所有者的原话：*"三态本身就是一个进入时"*、*"电脑上的三态设备，是随着智能体的"*、
*"平常是自己运行的"*、*"和解耦又耦合"*。

正确的理解：**三态是智能体进入某个载体并在其上显现的那一段过程，随智能体走，
不是载体自己的属性。**

- **解耦** —— 智能体的运行不依赖任何一台设备。电脑关了，它在别处照跑。
- **耦合** —— 它一旦要在电脑上动手，就必须走这套三态，因为那是「进入」的协议。

所以问题不是「三态归谁所有」，而是：**三态是一份进入协议（entry protocol），
任何载体上的显现都走它**。电脑走一份，手机走一份；载体可换，协议不变。
...
元层            —— 管「要不要进 / 进哪个载体 / 进多深」
  ↓
智能体本体      —— 一直在运行；云端/后台那条线不需要进入任何载体
  ↓ 进入协议（三态）
载体上的显现    —— 电脑 / 手机 / 其他，各自走一遍 SILENT→LIMINAL→MANIFEST→SILENT
```
