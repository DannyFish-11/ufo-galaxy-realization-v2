...
class UIController:
    ...
    def show_island(self):
        ...
        self._notify_ui_change("island", True)
    ...
    def show_sidesheet(self):
        ...
        self._notify_ui_change("sidesheet", True)
    ...
    def show_fullagent(self):
        ...
        self._notify_ui_change("fullagent", True)
    ...
    def hide_all(self):
        ...
        self._notify_ui_change("none", False)

    def _notify_ui_change(self, component: str, visible: bool):
        """
        Notify UI rendering subsystem of state changes.

        Dispatches the UI change event via the event bus if available,
        or logs the change for external renderers to poll.
        """
        state = {
            "component": component,
            "visible": visible,
            "island": self.island_visible,
            "sidesheet": self.sidesheet_visible,
            "fullagent": self.fullagent_visible,
        }
        # Try event bus dispatch
        try:
            import importlib
            events_mod = importlib.import_module("core.event_bus")
            if hasattr(events_mod, "emit"):
                events_mod.emit("ui_state_changed", state)
                return
        except (ImportError, AttributeError):
            pass
        # Fallback: log state for external renderers
        logger.debug(f"UI state: {state}")

    def get_visible_component(self) -> Optional[str]:
        """Get the currently visible UI component name, or None."""
        if self.fullagent_visible:
            return "fullagent"
        if self.sidesheet_visible:
            return "sidesheet"
