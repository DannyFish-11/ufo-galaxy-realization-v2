...
class OperationalStateSurface:
    ...
    def render(self, projection: Dict[str, Any]) -> str:
        ...
        lines: List[str] = [
            _ansi.c("  ┌─ Operational State Board ───────────────────┐", _BOLD),
            f"  │  Contract : {_clip(str(board.get('authority') or 'unknown'))}",
            f"  │  Version  : {board.get('contract_version') or 'unknown'}",
        ]
        for item in categories:
            label = str(item.get("label") or item.get("category_id") or "state")
            state = str(item.get("state") or "unknown")
            boundary = str(item.get("source_of_truth_boundary") or "unknown")
