...
#
# 修的是什么:CommandRouter —— 全系统**唯一**的规范派发口 —— 此前从不询问 V1。
# 也就是说「会话连续性合法性」这条权威，在结果回流侧(galaxy_gateway/android/
# handlers/task_lifecycle.py 的 TERMINAL_RESULT_INGESTION)守着，在**下发侧**
# 完全没有守。后果很具体:一台设备的 runtime attachment 已经进入 terminal 态
# (replaced / invalidated —— 例如同一设备重新登录、会话被顶替),它仍然可以被
# 派发新命令；直到命令执行完、结果回来，才在回流口被拒。中间那次真实执行已经
# 发生了，拒不回来。
#
# V1 与 Gate A 不是重复实现,它们问的是两个不同的问题:
#   Gate A(source_execution_eligibility):这个**来源**有没有执行姿态?
#                                        —— control_only 的手机不该本地执行。
...
# 这不是"放松"，是两种判定语义本来就不同:
#   REJECT         = 权威**做出了判定**:身份已失效(terminal 态 / attachment
#                    id 不匹配)。这正是审计要求必须拒的情形 → 硬阻断。
#   REQUIRE_REVIEW = 权威**没能判定**。在本路径上它只有一个来源:
#                    core.attached_runtime_session_registry 导入失败。
#                    把"权威自己坏了"升级成"全系统一条命令都发不出去"，
#                    是自伤而不是防护 —— Gate A / Gate B 在同样处境下也都是
#                    降级放行 + 留痕，这里保持一致。
# 想要更严的部署可以显式打开(见下方 env)，默认不替运维做这个决定。
...
class CommandRouter:
    ...
    def _build_dispatch_continuity_context(envelope: "TaskEnvelope") -> Any:
        ...
        这样同一个 envelope 在两个门里被认成同一个身份,不会出现"V3 认得、V1 不
        认得"的错位。

        缺字段不是错误:V1 对空身份的判定是 ALLOW(见 ``_check_session_identity``
    ...
    def _evaluate_dispatch_continuity_legality(
        ...
        """Gate C:向 V1 权威提交本次派发,返回 ``None`` 放行 / 结构化错误则阻断。

        阻断策略见 :data:`DISPATCH_CONTINUITY_LEGALITY_GATE_APPLIED`:
        ``REJECT`` 恒阻断;``REQUIRE_REVIEW`` 仅在
        ``GALAXY_DISPATCH_CONTINUITY_LEGALITY_ENFORCEMENT=strict`` 时阻断。

        权威本身不可用(导入失败/内部异常)时降级放行并在 trace 里留痕 ——
        与 Gate A / Gate B 的降级语义一致。
        """
        try:
            ...
            report = evaluate_continuity_legality(
                ...
                self._build_dispatch_continuity_context(envelope),
            )
            ...
            # 降级放行:留痕而不是静默,这样"门没跑"和"门跑了并放行"可区分。
        ...
        _strict = os.environ.get(DISPATCH_CONTINUITY_LEGALITY_ENFORCEMENT_ENV, "").strip().lower() == "strict"
        _should_block = verdict is ContinuityLegalityVerdict.REJECT or (
            _strict and verdict is ContinuityLegalityVerdict.REQUIRE_REVIEW
        )
        ...
        if not _should_block:
            if verdict is ContinuityLegalityVerdict.REQUIRE_REVIEW:
                logger.warning(
                    ...
                    "REQUIRE_REVIEW (权威未能判定,非阻断) reason=%s task_id=%s "
                    "—— 需要阻断请设置 %s=strict",
        ...
        return {
            ...
            "error_message": (
                f"会话连续性合法性校验未通过（{verdict.value}）：" f"{report.reject_reason or '无附加原因'}"
            ),
            "latency_ms": 0.0,
