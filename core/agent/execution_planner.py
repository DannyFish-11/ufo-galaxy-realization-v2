...
# PR-19: sentinel confirming memory runtime bias is wired into ExecutionPlanner.
MEMORY_BIAS_PLANNER_CONTINUITY_WIRED_PR19: str = (
    ...
    "PlannerContinuityGuidance to softly bias strategy selection based on "
    "memory posture (continuity_seeking / retrieval_seeking / novelty).  "
    "Hard gates (governance, activation context) and explicit user intent "
    "(task_type mapping table) remain authoritative.  Memory bias adjusts "
    "complexity thresholds only after the task_type and keyword checks."
)
...
class ExecutionPlanner:
    ...
    async def execute(self, plan: ExecutionPlan, *, activation_budget: Optional[Any] = None, memory_bias: Optional[Any] = None) -> ExecutionResult:
        ...
        Args:
            ...
            memory_bias:        PR-19 — optional MemoryRuntimeBias from
                                core.cognitive.memory_runtime_bias.
                                When provided, planner continuity guidance is
                                derived and used to softly bias strategy
                                selection based on memory posture.  Explicit
                                user intent (task_type table) and hard
                                governance gates are unaffected.
        """
        t0 = time.monotonic()
        ...
        try:
            if memory_bias is not None and getattr(memory_bias, "influenced_by_memory", False):
                from core.cognitive.memory_runtime_bias import (  # noqa: PLC0415
                    get_planner_continuity_guidance as _get_continuity,
                )
                _continuity_guidance = _get_continuity(memory_bias)
                logger.debug(
                    "PR-19 ExecutionPlanner: continuity_guidance=%s strategy_bias=%s adj=%.2f",
                    _continuity_guidance.posture,
                    _continuity_guidance.strategy_bias,
                    _continuity_guidance.complexity_threshold_adjustment,
                )
    ...
    def _pick_strategy(self, message: str, complexity: float, task_type: str = "", breadth_guidance: Optional[Any] = None, continuity_guidance: Optional[Any] = None) -> str:
        ...
        PR-18 activation budget influence (advisory):
        ...
        PR-19 memory continuity influence (advisory, subordinate to PR-18):
          - continuity_seeking:  strategy_bias from prior memory; raises
                                 complexity thresholds (+0.10) to prefer the
                                 already-proven strategy.
          - retrieval_seeking:   strategy_bias="team"; slightly lowers thresholds
                                 (-0.05) to enable retrieval-capable strategies.
          - novelty:             no strategy bias; no threshold adjustment.
        ...
        # PR-19: apply complexity threshold adjustment from continuity guidance
        # (subordinate to PR-18 breadth guidance — added on top)
        _memory_strategy_bias = None
        if continuity_guidance is not None and getattr(continuity_guidance, "influenced_by_memory", False):
            _mem_adj = float(getattr(continuity_guidance, "complexity_threshold_adjustment", 0.0))
            _memory_strategy_bias = getattr(continuity_guidance, "strategy_bias", None)
            if _mem_adj != 0.0 or _memory_strategy_bias:
                adj += _mem_adj  # stack on top of PR-18 adjustment
                logger.debug(
                    "PR-19 _pick_strategy: posture=%s mem_adj=%.2f strategy_bias=%s total_adj=%.2f",
                    getattr(continuity_guidance, "posture", "?"),
                    _mem_adj,
                    _memory_strategy_bias,
                    adj,
                )
        ...
        if _strategy_pref == "single":
            ...
            return "single"
        # PR-19: if memory bias suggests team strategy (retrieval_seeking), return team
        if _memory_strategy_bias == "team":
            logger.debug("PR-19 _pick_strategy: retrieval_seeking — returning 'specialized' per memory bias")
            return "specialized"
        return "single"
