...
class EndToEndPipeline:
    ...
    async def execute(
        ...
        try:
            if self.session_manager:
                ...
                if session_id:
                    ...
                    session = await self.session_manager.get_or_create_session(user_id, source_device_id)
                    session_id = session.id if session else None

                if session_id:
                    await self.session_manager.add_message(session_id, "user", message, source_device_id)
                    result["session_id"] = session_id

                    # 从历史补充 context
                    if not context:
                        context = self.session_manager.get_history(session_id, max_turns=10)
        ...
        try:
            ...
            if self.llm_router and self.llm_router.is_available():
                ...
                for t in parsed.targets:
                    if isinstance(t, str) and any(kw in t for kw in ["phone", "手机", "平板", "tablet", "pc", "电脑"]):
                        target_device = t
                        break
        except Exception as e:
        ...
        if intent_type in ("device_control", "multi_device") and target_device:
            ...
            result = await self._execute_device_control(message, target_device, source_device_id, intent_data, result)
        elif intent_type in ("action", "workflow", "batch_task", "multi_device"):
            # 路径 B: 复杂任务 → Agent 工厂
            result = await self._execute_agent_task(message, intent_data, context, result)
        else:
            # 路径 C: 纯聊天 → LLM 直接回复
        ...
        if session_id and self.session_manager:
            try:
                await self.session_manager.add_message(session_id, "assistant", result["reply"], "")
                ...
                if session:
                    for device_id in session.devices:
                        if device_id != source_device_id:
                            await self.connection_manager.send_to_device(
                                device_id,
                                {
                                    "type": "chat_response",
    ...
    async def _execute_device_control(
        ...
        try:
            ...
            if self.connection_manager:
                sent = await self.connection_manager.send_to_device(
                    target_device,
                    {
                        "type": "agent_manifest",
                ...
                if sent:
                    result["success"] = True
                    result["reply"] = f"已将控制 Agent 分发到设备 {target_device}，正在执行: {message}"
    ...
    async def _execute_agent_task(
        ...
        try:
            ...
            if len(targets) > 2:
                # 高复杂度 → 分形分解
                try:
                    fractal_result = await self.agent_factory.create_fractal_task(message, intent_data)
            ...
            # 标准 → LLM 动态生成 Agent
            agent = await self.agent_factory.create_from_llm(message, context=intent_data)
    ...
    async def _execute_chat(
        ...
        if not self.llm_router or not self.llm_router.is_available():
            result["reply"] = (
                "LLM 服务未配置。请在 Dashboard 设置 API Key，" "或在 .env 中设置 OPENAI_API_KEY / DEEPSEEK_API_KEY。"
            )
            return result
        ...
        try:
            ...
            response = await self.llm_router.chat_completion(messages=messages, task_type="GENERAL")
            result["success"] = True
