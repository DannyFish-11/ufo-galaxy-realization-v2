...
-------------
  ...
  C 档  双模型（复合，全部同时跑） —— 感知位 MiniCPM-o 4.5 + 推理位 Qwen3.6-35B-A3B
                                      （推理位第二候选：Agents-A1 · 35B-A3B，智能体专精）
...
_MODELS: Dict[str, ModelSpec] = {
    ...
    "agents-a1:35b-a3b": ModelSpec(
        "agents-a1:35b-a3b",
        "Agents-A1 · 35B-A3B",
        "MoE 推理位:智能体专精(长程任务拆解 · 工具调用 · 指令遵循)",
        ModelCapability(vision=False, audio_in=False, audio_out=False, tools=True),
        # 与 qwen3.6:35b-a3b 同一条加载路:同为 35B-A3B,显存账同样建立在专家卸载上。
        # 正因为门槛一样,它是 C 档的**第二候选**而不是新开一档 —— 档是硬件门槛的
        # 单位(见 _REASONING_CANDIDATES 与 _TIER_KEYS 的说明),D 档单列是因为 9B
        # 稠密不欠专家卸载这张票,门槛差着一整级;这一位不是。
        source="llama_cpp",
        requires_gpu=True,
        # **没量过,填的是保守值。** 官方 GGUF 是 InternScience/Agents-A1-Q4_K_M-GGUF,
        # 但那一档的真实体积我没核到(社区同族 UD-Q4_K_XL 约 21 GB,拿它当参考点)。
        # 往大了填是安全方向:准入宁可判"放不下"也不能判成放得下再加载到一半 OOM。
        # 权重真落到盘上之后 effective_weight_mb() 会用 stat 的真值压过这一栏,
        # 差过 15% 还会报"你换过量化"——所以这个数只在"还没下载"时说话。
        size_mb_val=21000,
        # **故意留 0。** 0 → runtime_mb() 退回 size_mb(),即"假设专家卸载一点便宜
        # 都没占到"。qwen3.6 那位敢写 7300 是因为**实测过**;这一位没有人量过,
        # 照抄同架构的 7300 就是把一张空头支票当成事实 —— 那正是 runtime_mb_val
        # 这一栏存在的理由(见它自己的注释:"没量过就退回权重大小,保守但至少不是
        # 编出来的")。
        #
        # 留 0 有**两个后果,方向相反**,读的人必须一起知道:
        #
        # 1. 准入侧(``tier_runtime_footprint_range_mb``)变成 lo == hi == 整权重
        #    —— 小卡上直接判"这一档跑不起来"。这是对的:没量过就不该声称放得下。
        # 2. ``in_process_cannot_serve`` 的第一条判据是"MoE 且 runtime < 整权重",
        #    留 0 会让它取假 → **不会**因为进程内绑定缺 ``--n-cpu-moe`` 而改判
        #    ``llama_server``。也就是说在够大的卡上,它会走进程内、不开专家卸载。
        #    这是留 0 的**真实代价**,不是疏漏 —— 一旦量出真值填进来就自动解除。
        #
        # 怎么量:目录顶上那套 ``register_ephemeral_spec``(临时挂载,不落库)就是
        # 为这件事准备的 —— "验证一个新架构能否被调度器正确拆分并加载"。挂上去
        # 量到驻留量,再回来填这一栏,上面两条一起归位。
        runtime_mb_val=0,
        is_moe=True,
        # 底座是 Qwen3.5-35B-A3B,原生 262144 —— 与 qwen3.6 那位同口径:记**原生**
        # 上限,不记 YaRN 外推。
        max_ctx_val=262144,
        # 没量过。同上:0 会让上下文预算退回"只认模型上限、不敢按显存放大"。
        kv_mb_per_1k_val=0,
    ),
...
_REASONING_CANDIDATES: List[str] = [
    # **第一个是默认**(见 BrainSlot.tag)。现任排在前,所以加入第二候选不改变
    # 任何既有行为 —— 只有用户显式选过(main_brain())才会换人。
    "qwen3.6:35b-a3b",
    # 智能体专精版,底座 Qwen3.5-35B-A3B。与现任同架构、同专家卸载、同 262144,
    # 所以是**同一位的另一个候选**,不是新的一档。
    #
    # 加候选时那句"不同型号的专家卸载拆分结果不一样,换完必须重走 _split_moe"
    # 在这里第一次真的生效:在此之前这张表只有一个元素,那句话没有对手。
    "agents-a1:35b-a3b",
]
...
_TIERS: Dict[str, Tier] = {
    ...
    "C": Tier(
        "C",
        "C 档 · 双模型 · 35B 推理位",
        "推理位二选一(Qwen3.6-35B-A3B 通用 / Agents-A1 智能体专精，同为 MoE，都要专家卸载)常驻独显；感知位四选一(Gemma 4 系 / MiniCPM-o)走核显，两位都可随时换",
        "composite",
        [
