...
* :mod:`tools.architecture.architecture_diagnostics` reports
...
class RuntimeIntrospectionSnapshot:
    ...
    fallback_policy:
        ...
        available.
    diagnostics_report:
        Serialised :class:`~tools.architecture.architecture_diagnostics.DiagnosticsReport`
        dict, when available.  Generated by
        :func:`~tools.architecture.architecture_diagnostics.run_architecture_diagnostics`.
...
def _run_diagnostics(
    ...
    try:
        # 直接指向规范实现。原先经 ``tools.architecture.architecture_diagnostics`` 兼容 shim 转发，
        # 那个 shim 自己的 docstring 就写着「已迁移到 tools.architecture，将在未来版本
        # 移除，请直接从新位置导入」—— 它已随本批零引用兼容层一并删除。
