...
class GalaxyCore:
    ...
    async def call_node(self, node_id: str, action: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await client.post(f"{endpoint}/mcp/call", json={"tool": action, "params": params}, timeout=10.0)
            ...
            response = await client.post(f"{endpoint}/{action}", json=params, timeout=10.0)
            ...
            logger.error(
                f"调用节点 {node_id} ({node.get('name', '?')}) HTTP错误: status={e.response.status_code}, action={action}"
            )
    ...
    async def call_node_with_protocol(self, node_id: str, action: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            response = await client.post(f"http://{self._node_host}:{port}/protocol/message", json=message.to_dict())
    ...
    async def register_device(self, device_id: str, device_type: str, name: str, endpoint: str = "") -> Dict[str, Any]:
        """
    ...
    async def send_device_command(self, device_id: str, command: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        ...
        if device_type == "android":
            ...
            return await self.call_node(
                "92", command, {"device_id": device_id, "platform": "android", **(params or {})}
            )
    ...
    async def smart_call(self, capability: str, action: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        ...
        if self._master_brain is not None:
            ...
            if p.get("_distributed") or p.get("target_worker_id"):
                try:
                    result = await self._master_brain.execute_distributed_task(
                        {
        ...
        return await self.call_node("04", "route", {"capability": capability, "action": action, "params": params or {}})

    # =========================================================================
    # 自主能力 - 调用已有节点
    # =========================================================================

    async def autonomous_learn(self, experience: Dict[str, Any]) -> Dict[str, Any]:
        """自主学习 - 调用 Node_70"""
        return await self.call_node("70", "record_experience", experience)

    async def autonomous_think(self, goal: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        """自主思考 - 调用 Node_108"""
        return await self.call_node("108", "reflect", {"goal": goal, "context": context or {}})

    async def autonomous_code(self, task: str, files: List[str] = None) -> Dict[str, Any]:
        """自主编程 - 调用 Node_101"""
        return await self.call_node("101", "generate_code", {"task": task, "target_files": files or []})

    async def query_knowledge(self, query: str, top_k: int = 5) -> Dict[str, Any]:
        """查询知识库 - 调用 Node_72"""
        return await self.call_node("72", "search", {"query": query, "top_k": top_k})
