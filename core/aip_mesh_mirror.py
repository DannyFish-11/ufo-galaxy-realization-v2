...
_MESH_EXEMPT: Dict[str, str] = {
    ...
    "agent_message": (
        "载的是对话正文 —— 智能体跟用户说的那句话。镜像到网格上就是把一段私人对话"
        "广播给每一个节点,和 voice_event 是同一类问题(那条也因此豁免)。\n"
        "用户要的「同一套上下文跨设备」不是靠网格广播实现的:那是**定向投递**给"
        "这个人自己的几台设备,走网关的按设备下发,由 conversation_id 归拢。"
        "两者的收件人完全不同 —— 一个是他的手机手表,一个是网格上所有节点。"
    ),
    "execution_proposal": (
        "候选集合是**安全边界**,不能绕过去。相位 1 已经按 peer_trust 与 CapabilityTier "
        "把候选池滤过一遍了 —— blocked 的对端是硬拒绝。把提议广播到网格,等于让一个"
        "被拉黑的对端看见我们**正要做什么**,那道过滤就白做了。\n"
        "提议要发给谁,是算出来的,不是喊一嗓子看谁应。"
    ),
    "execution_commitment": (
        "定向回执:它认领的是某一轮提议,收件人只有发起那轮的中心。"
        "广播出去,网格上每个节点都会看到某台设备此刻忙不忙、就位没有 —— "
        "那是设备状态,该由 state_event 按在场/可打断性表达,不是把协商回执原样转发。"
    ),
    "execution_commit": (
        "同 execution_proposal:收件人恰好是当初被提议到的那几台(**包括落选的**,"
        "它们必须知道自己出局,否则会一直占着为这次协商留的资源)。"
        "范围之外的节点从没收到过提议,给它们发落定没有意义。"
    ),
    "decision_withdraw": (
        "定向投递,不是广播。它要收回的那几条通知,只存在于当初被**分叉到**的那几台"
        "设备上(见 pending_decision_registry 的 _discover_target_devices)。"
        "网格上其余节点从没收到过那条 decision_request,也就没有东西可收 —— "
        "广播给它们只是噪声。\n"
        "更要紧的是收件人不同:分叉的范围是「这个人此刻连着的几台设备」,"
        "网格的范围是「所有节点」。把前者当成后者,就是又一次把私人范围的东西"
        "扩散到公共范围 —— 和 agent_message 豁免的理由同源。"
    ),
    "voice_call_end": (
