...
两条判据上的讲究
================
* **只数正增量。** 压缩会让消息列表**大幅变短**，把那个负的差值算进去，烧率会被
  拉到接近 0，跑道随之变成"还能跑很远" —— 于是压完一次就再也不压了。这是个会自己
  失效的闭环，和 ``context_measurements`` 里那条"只记不受 n_ctx 反向约束的量"
  是同一类陷阱。
* **取最近几轮的最大值，不是平均。** 方向性后果不对称：烧率估小 → 跑道估长 →
  **撞墙被静默截断**；估大 → 提前压一次 → 丢一点细节。取最大偏保守，而"最近几轮"
  让它在任务节奏变了之后能跟着降下来，不会被早期一个异常值永久钉死。
...
def burn_per_round(marks: Optional[List[int]]) -> int:
    ...
    **只数正增量**：压缩会让占用大幅回落，把那个负差值算进去会把烧率拉到接近 0，
    于是跑道变成"还能跑很远"，压完一次就再也不压了 —— 一个会自己失效的闭环。
    """
