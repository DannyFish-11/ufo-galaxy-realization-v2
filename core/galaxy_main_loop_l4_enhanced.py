"""
core.galaxy_main_loop_l4_enhanced — Canonical L4 runtime module
================================================================

This is the **canonical** home of the L4 runtime classes.  The deprecated
root-level ``galaxy_main_loop_l4.py`` has been retired; all code that
previously imported from there should import from here instead.
...
L4 capabilities are managed through ``unified_launcher.py``.
The authoritative system entrypoint is ``main.py`` → ``unified_launcher.py``.
"""
...
# Ensure project root is on sys.path for optional enhancement imports
_project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if _project_root not in sys.path:
    sys.path.insert(0, _project_root)

# ---------------------------------------------------------------------------
# L4 optional component imports (each degrades independently)
# ---------------------------------------------------------------------------
...
try:
    ...
    _logger.warning(f"学习模块不可用（缺少依赖: {_e}），L4 将以基础模式运行")
...
# ---------------------------------------------------------------------------
# Enhanced subclass (canonical public interface)
# ---------------------------------------------------------------------------

class GalaxyMainLoopL4Enhanced(GalaxyMainLoopL4):
    """
    Canonical L4 runtime class.

    Extends ``GalaxyMainLoopL4`` with synchronous goal-submission helpers
    needed by the UI / WebSocket layer.

    Canonical import::

        from core.galaxy_main_loop_l4_enhanced import GalaxyMainLoopL4Enhanced

    Extra methods vs base:
    - ``receive_goal(description)`` → ``goal_id`` (sync; for UI / WebSocket callers)
    """

    def receive_goal(self, goal_description: str, goal_type: str = "task_execution") -> str:
        """
        Synchronously accept a goal and enqueue it for the L4 main loop.

        Args:
            goal_description: Natural-language goal description.
            goal_type: Goal-type string (default ``"task_execution"``).

        Returns:
            Generated goal-ID string (usable for subsequent status queries).
        """
        # Lazy import: enhancements.reasoning depends on optional packages (numpy, etc.)
...
class PendingGoal:
    """
    Lightweight pending-goal container.

    Used by the WebSocket service layer and UI layer to buffer goals before
    submitting them to the L4 main loop.
    """
    description: str
...
def get_galaxy_loop(config: Optional[Dict[str, Any]] = None) -> GalaxyMainLoopL4Enhanced:
    """
    Factory: return a new (uninitialised) ``GalaxyMainLoopL4Enhanced`` instance.

    Args:
        config: Optional configuration dict forwarded to ``GalaxyMainLoopL4``.

    Returns:
        A freshly constructed (not yet started) ``GalaxyMainLoopL4Enhanced``.
    """
    return GalaxyMainLoopL4Enhanced(config or {})


__all__ = [
    # Base class (canonical location)
    "GalaxyMainLoopL4",
    # Data types
    "CycleState",
    "L4CycleResult",
    # Enhanced subclass (primary public interface)
    "GalaxyMainLoopL4Enhanced",
    "PendingGoal",
    "get_galaxy_loop",
    # Availability flags (re-exported for test/introspection use)
