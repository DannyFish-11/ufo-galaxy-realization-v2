...
nodes_root: str = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "nodes")
...
def _load_node(node_id: str, node_dir: str, fusion_entry_path: str) -> Optional[Dict]:
    ...
    spec = importlib.util.spec_from_file_location(
        f"nodes.{node_id}.fusion_entry", fusion_entry_path, submodule_search_locations=[node_dir]
    )
...
async def _execute_node(node_info: dict, action: str, params: dict):
    ...
    if node_info["type"] == "function":
        ...
        if inspect.iscoroutinefunction(func):
            ...
            return await asyncio.get_running_loop().run_in_executor(None, func, action, params)
        ...
        if inspect.iscoroutinefunction(method):
            ...
            return await asyncio.get_running_loop().run_in_executor(None, lambda: method(action, **params))
