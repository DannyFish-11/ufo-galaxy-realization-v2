...
def create_router(service_manager=None, config=None) -> APIRouter:  # noqa: ARG001
    ...
    # ==================================================================
    # Task-6: Operator Action Closure Chain endpoints
    # ==================================================================
    #
    # These endpoints expose the minimal action → ack → truth → closure
    # traceable chain to operator consumers.  They are *read-only* from
    # an authority perspective: they surface what the canonical closure
    # chain recorded, but do not grant new authority to the operator plane.
    #
    # Endpoints:
    #   GET  /api/v1/operator/closure-chain/summary
    #   GET  /api/v1/operator/closure-chain/records
    #   GET  /api/v1/operator/closure-chain/{action_id}
    #   POST /api/v1/operator/closure-chain/{action_id}/subject-ack
    #   POST /api/v1/operator/closure-chain/{action_id}/truth-convergence
    #   POST /api/v1/operator/closure-chain/{action_id}/verify-closure

    # ------------------------------------------------------------------
    # GET /api/v1/operator/closure-chain/summary
    # ------------------------------------------------------------------

    @router.get("/api/v1/operator/closure-chain/summary")
    async def operator_closure_chain_summary() -> JSONResponse:
        """Return a summary of all open and closed operator action closure chains.
        ...
        """
        try:
            from core.operator_action_closure_chain import get_closure_chain_summary
            summary = get_closure_chain_summary()
            return JSONResponse(content={**summary, "operator_routes_authority": "OPERATOR_ROUTES_V1"})
        except Exception as exc:
            logger.error("operator_closure_chain_summary endpoint error: %s", exc)
            return JSONResponse(
                content={"error": str(exc), "authority": "OPERATOR_ROUTES_V1"},
                status_code=500,
            )
    ...
    @router.get("/api/v1/operator/closure-chain/records")
    async def operator_closure_chain_records(
        limit: int = Query(50, ge=1, le=500),
        stage: Optional[str] = Query(None),
        action_kind: Optional[str] = Query(None),
    ) -> JSONResponse:
        """Return closure chain records newest-first.

        Query parameters:
          - ``limit``: Max records (default 50, max 500).
          - ``stage``: Filter by :class:`~core.operator_action_closure_chain.ClosureChainStage`.
          - ``action_kind``: Filter by action kind.

        Returns a list of serialised :class:`~core.operator_action_closure_chain.ClosureChainRecord`.
        """
        try:
            from core.operator_action_closure_chain import get_all_closure_chain_records
            records = get_all_closure_chain_records(
                limit=limit, stage=stage, action_kind=action_kind
            )
            return JSONResponse(content={
                "records": [r.to_dict() for r in records],
                "count": len(records),
            ...
            logger.error("operator_closure_chain_records endpoint error: %s", exc)
            return JSONResponse(
                content={"error": str(exc), "authority": "OPERATOR_ROUTES_V1"},
                status_code=500,
            )
    ...
    @router.get("/api/v1/operator/closure-chain/{action_id}")
    async def operator_closure_chain_record(action_id: str) -> JSONResponse:
        """Return the closure chain record for a specific action_id.

        Returns HTTP 404 when the action_id is not found.
        """
        try:
            ...
            if record is None:
                return JSONResponse(
                    content={"error": f"action_id={action_id} not found", "authority": "OPERATOR_ROUTES_V1"},
                    status_code=404,
                )
            return JSONResponse(content={**record.to_dict(), "operator_routes_authority": "OPERATOR_ROUTES_V1"})
        except Exception as exc:
            logger.error("operator_closure_chain_record endpoint error: %s", exc)
            return JSONResponse(
                content={"error": str(exc), "authority": "OPERATOR_ROUTES_V1"},
                status_code=500,
            )
    ...
    @router.post("/api/v1/operator/closure-chain/{action_id}/subject-ack")
    async def operator_closure_chain_subject_ack(
        action_id: str, body: Dict[str, Any]
    ) -> JSONResponse:
        """Record that a subject (device/agent/participant) has acked an action.

        Body::

            {
              "subject_id": str,
              "ack_state": str,       // "acked" | "nacked" | "skipped" | "timeout"
              "ack_payload": dict,    // optional
              "notes": [str]          // optional
            }

        Returns the updated closure chain record.
        """
        try:
            ...
            if not subject_id:
                return JSONResponse(
                    content={"error": "subject_id is required", "authority": "OPERATOR_ROUTES_V1"},
                    status_code=422,
                )
            updated = record_subject_ack(
                action_id=action_id,
                subject_id=subject_id,
                ack_state=str(body.get("ack_state", "acked")),
                ack_payload=dict(body.get("ack_payload") or {}),
            ...
            return JSONResponse(content={**updated.to_dict(), "operator_routes_authority": "OPERATOR_ROUTES_V1"})
        except Exception as exc:
            logger.error("operator_closure_chain_subject_ack endpoint error: %s", exc)
            return JSONResponse(
                content={"error": str(exc), "authority": "OPERATOR_ROUTES_V1"},
                status_code=500,
            )
    ...
    @router.post("/api/v1/operator/closure-chain/{action_id}/truth-convergence")
    async def operator_closure_chain_truth_convergence(
        action_id: str, body: Dict[str, Any]
    ) -> JSONResponse:
        """Record that canonical truth has converged for an action's effects.

        This endpoint is called by canonical truth modules (NOT by the operator
        UI).  The operator/control plane MUST NOT be the authority for truth
        convergence; this endpoint only records what canonical truth modules
        report.

        Body::

            {
              "truth_convergence_payload": dict,
              "canonical_truth_refs": [str],
        ...
        try:
            ...
            updated = record_truth_convergence(
                action_id=action_id,
                truth_convergence_payload=dict(body.get("truth_convergence_payload") or {}),
                canonical_truth_refs=list(body.get("canonical_truth_refs") or []),
                truth_chain_complete=bool(body.get("truth_chain_complete", False)),
                notes=list(body.get("notes") or []),
            )
            if updated is None:
                return JSONResponse(
            ...
            logger.error("operator_closure_chain_truth_convergence endpoint error: %s", exc)
            return JSONResponse(
                content={"error": str(exc), "authority": "OPERATOR_ROUTES_V1"},
                status_code=500,
            )
    ...
    @router.post("/api/v1/operator/closure-chain/{action_id}/verify-closure")
    async def operator_closure_chain_verify_closure(
        action_id: str, body: Dict[str, Any]
    ) -> JSONResponse:
        """Verify and finalize the closure of an operator action.

        Closure is considered verified when all routed subjects have acked
        (or been accounted for) AND canonical truth confirms the action's
        effects.

        Body::

            {
              "closure_verdict": str,   // "success" | "partial" | "failed" | "policy_blocked" | "requires_review"
              "closure_payload": dict,  // optional
              "notes": [str]            // optional
            }

        Returns the updated closure chain record.  ``closure_verified`` will
        be ``true`` when all conditions for closure are met.
        """
        try:
            from core.operator_action_closure_chain import (
                verify_closure,
            )
            closure_verdict = str(body.get("closure_verdict", "success")).strip()
            updated = verify_closure(
                action_id=action_id,
                closure_verdict=closure_verdict,
                closure_payload=dict(body.get("closure_payload") or {}),
                notes=list(body.get("notes") or []),
            )
            if updated is None:
                return JSONResponse(
            ...
            logger.error("operator_closure_chain_verify_closure endpoint error: %s", exc)
            return JSONResponse(
                content={"error": str(exc), "authority": "OPERATOR_ROUTES_V1"},
                status_code=500,
            )
