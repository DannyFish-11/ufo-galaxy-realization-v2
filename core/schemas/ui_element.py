...
**输出**(点击/输入),没用它做**输入**(理解界面)。本契约把这棵树结构化,让模型对着
「名为『发送』的按钮」推理,而不是对着像素猜——**结构优先**。

结构与视觉并存(不是二选一)
--------------------------
- 有系统 API 结构 → 用它(准、状态明确)。
- 第三方 App 封锁无障碍 / Flutter·Canvas·游戏 树是空的 → 回退视觉(VLM/OCR)。
- 二者可 :meth:`UIGraph.merge` 融合成一张**混合图**(source 各自标注),视觉补上结构
  缺失的控件、结构给视觉控件补上语义与可交互性——两条腿一起走。
