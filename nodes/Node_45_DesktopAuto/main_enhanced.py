...
"""
    ...
    try:
        # 使用 Gemini 分析
        response = llm_client.models.generate_content(model="gemini-2.0-flash-exp", contents=[query, screenshot])
...
async def health():
    return {
        ...
        "version": "3.0.0",
        "features": {"pyautogui": pyautogui is not None, "llm": llm_client is not None, "opencv": cv2 is not None},
...
async def analyze_screen(request: AnalyzeScreenRequest):
    ...
    if analysis:
        return {"success": True, "query": request.query, "analysis": analysis}
    else:
        return {"success": False, "error": "Failed to analyze screen"}
...
async def smart_click(request: SmartClickRequest):
    ...
    if result and result.get("found"):
        ...
        try:
            ...
            return {"success": True, "element": result.get("element"), "x": x, "y": y, "clicks": request.clicks}
        except Exception as e:
            return {"success": False, "error": str(e)}
    else:
        return {"success": False, "error": f"Element not found: {request.description}"}
