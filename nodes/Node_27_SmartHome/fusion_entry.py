...
logger = logging.getLogger("Node_27_SmartHome")
...
def _import_from_node_main(class_name):
    ...
    spec = importlib.util.spec_from_file_location(
        "Node_27_SmartHome.main", main_path,
...
def _get_service():
    ...
    if _service is None:
        try:
            cls = _import_from_node_main("SmartHomeService")
            _service = cls()
            logger.info("Node_27_SmartHome 服务已初始化")
        except Exception as e:
            logger.error(f"Node_27_SmartHome 服务初始化失败: {e}")
            raise
    return _service
...
def execute(action: str, params: dict = None) -> dict:
    ...
    try:
        ...
        if action == "status":
            return {
                "node": "Node_27_SmartHome",
                "status": "running",
                "service": type(service).__name__,
                "available_actions": ['discover', 'control', 'status', 'scene', 'schedule']
            }
        
        if action == "help":
            return {
                "node": "Node_27_SmartHome",
                "description": "智能家居控制节点",
                "actions": {'discover': '发现设备', 'control': '控制设备', 'status': '设备状态', 'scene': '场景控制', 'schedule': '定时任务'}
            }
