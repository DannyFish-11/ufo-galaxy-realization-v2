#!/usr/bin/env python3
"""
Node_71_MultiDeviceCoordination - 融合入口
多设备协调节点
...
def _import_from_node_main(class_name):
    ...
    spec = importlib.util.spec_from_file_location(
        "Node_71_MultiDeviceCoordination.main", main_path,
...
def _get_service():
    ...
    if _service is None:
        try:
            cls = _import_from_node_main("MultiDeviceCoordinator")
            _service = cls()
            logger.info("Node_71_MultiDeviceCoordination 服务已初始化")
        except Exception as e:
            logger.error(f"Node_71_MultiDeviceCoordination 服务初始化失败: {e}")
            raise
    return _service
    ...
    try:
        ...
        if action == "status":
            return {
                ...
                "available_actions": ['register_device', 'assign_task', 'sync_state', 'create_group', 'coordinate']
            }
        ...
        if action == "help":
            return {
                "node": "Node_71_MultiDeviceCoordination",
                "description": "多设备协调节点",
                "actions": {'register_device': '注册设备', 'assign_task': '分配任务', 'sync_state': '同步状态', 'create_group': '创建设备组', 'coordinate': '协调执行'}
            }
