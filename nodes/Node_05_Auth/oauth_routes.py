...
def register_oauth_routes(app: FastAPI):
    ...
    async def oauth_callback(
        ...
        try:
            ...
            if not user_info.get("email"):
                _missing = [f for f in ("email", "id", "name") if not user_info.get(f)]
                logger.warning("OAuth 用户信息不完整 provider=%s 缺失字段=%s", provider, _missing)
        ...
        logger.info("OAuth 登录成功(%s/callback)", provider)
    ...
    async def oauth_google_id_token(body: GoogleIdTokenRequest):
        ...
        logger.info("OAuth 登录成功(google/id_token)")
        return token_data
    ...
    async def oauth_github_code(body: GitHubCodeRequest):
        ...
        token_data = _generate_jwt_token(user_info)
        logger.info("OAuth 登录成功(github/code)")
        return token_data
