...
# 端口必须查**节点**表，不是**服务**表。
# 这里原来是 `get_service_port("node_66") or 8066` —— 那个键在
# config/unified_ports.yaml 的 services 段里根本不存在，每次都 KeyError，被下面的
# except 接住，于是实际生效的永远是 os.getenv("PORT", "8066")：
#   * 经启动器起（会传 PORT）→ 碰巧对；
#   * 裸跑 `python main.py`（没有 PORT）→ 落到字面量 8066。
...
PORT = resolve_node_port("Node_66_ConfigManager", 8065)
