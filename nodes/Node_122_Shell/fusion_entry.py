...
logger = logging.getLogger("Node_122_Shell")
...
def _get_service():
    ...
    if _service is None:
        try:
            cls = _import_from_node_main("ShellService")
            _service = cls()
            logger.info("Node_122_Shell 服务已初始化")
        except Exception as e:
            logger.error(f"Node_122_Shell 服务初始化失败: {e}")
            raise
    return _service
    ...
    try:
        ...
        if action == "status":
            return {
                ...
                "available_actions": ['execute', 'script', 'list_processes', 'kill']
            }
        
        if action == "help":
            return {
                "node": "Node_122_Shell",
                "description": "Shell 操作节点",
                "actions": {'execute': '执行命令', 'script': '执行脚本', 'list_processes': '列出进程', 'kill': '终止进程'}
            }
