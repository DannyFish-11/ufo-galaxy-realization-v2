...
节点记录以「结构契约」表达，不绑定某个具体配置类
------------------------------------------------
本模块此前写的是 ``from .config_manager import NodeConfig`` —— 把一个纯算法
（拓扑排序）钉死在某一个启动器的配置类上。``launcher/config_manager.py`` 按其
自身登记的退役条件删除后，这行 import 直接让整个模块 ``ModuleNotFoundError``。

真正的依赖并不是"那个类"，而是三个属性：``name`` / ``priority`` /
``dependencies``。所以这里改成显式声明一个 :class:`NodeSpec` 结构协议
（``typing.Protocol``，纯类型层、运行时零 import），谁满足谁就能传进来。
统一启动器把节点表交给它时不必先适配某个历史 dataclass。

注意 ``priority`` 是本模块**实际读取**的字段（见 ``_group_by_priority`` 与
``resolve_startup_order`` 的排序），不是凑数写上去的 —— 协议只列真读的字段。
"""
...
class NodeSpec(Protocol):
    """本模块对「一个节点」的全部要求 —— 仅这三个属性。

    刻意不含 ``port`` / ``group`` / ``health_check_path`` 之类：拓扑排序不看
    它们，写进协议只会凭空收紧调用方。
    """
...
class DependencyResolver:
    ...
    def resolve_startup_order(self, target_nodes: List[str], include_dependencies: bool = True) -> List[str]:
        """
    ...
    def _get_dependency_depth(self, node_id: str, cache: Optional[Dict[str, int]] = None) -> int:
        ...
        max_depth = max(self._get_dependency_depth(dep_id, cache) for dep_id in deps if dep_id in self.nodes)
    ...
    def validate(self) -> Dict[str, any]:
        ...
        result = {
            ...
            "stats": {
                ...
                "root_nodes": len([n for n in self.nodes if not self._dependency_graph.get(n.id, set())]),
                "max_depth": max((self._get_dependency_depth(n.id) for n in self.nodes.values()), default=0),
            },
        }
        ...
        for node_id, config in self.nodes.items():
            for dep_id in config.dependencies:
                if dep_id not in self.nodes:
                    result["warnings"].append(f"Node {node_id} depends on non-existent node {dep_id}")
