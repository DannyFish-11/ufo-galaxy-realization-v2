...
__all__ = [
    ...
    "SYSTEM_COMPLETION_CLARITY_PR12_SENTINEL",
]

SYSTEM_COMPLETION_CLARITY_PR12_SENTINEL: str = (
    "SYSTEM_COMPLETION_CLARITY_PR12_SENTINEL_V1: "
    "ArchitectureCompletionScorecard explicitly surfaces canonical vs "
    "transitional dimensions and a residual debt register so compatibility "
    "paths are not mistaken for final architecture closure."
)
...
class ArchitectureCompletionScorecard:
    ...
    def transitional_dimensions(self) -> List[DimensionScorecard]:
        ...
        return [
            d
            for d in self.dimensions
            if d.maturity_level not in (MaturityLevel.CANONICALIZED, MaturityLevel.COMPLETE)
    ...
    def residual_debt_register(self) -> List[str]:
        ...
        for d in self.transitional_dimensions():
            ...
            if d.recommended_next_actions:
                for action in d.recommended_next_actions:
                    entries.append(f"{dim_name}: next - {action}")
        return entries
    ...
    def completion_clarity_snapshot(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "canonical_boundary_note": (
                "Dimensions listed under canonical_dimensions represent the current "
                "authoritative architecture path."
            ),
            "compatibility_boundary_note": (
                "Dimensions listed under transitional_dimensions include compatibility "
                "or unfinished convergence surfaces and should not be treated as final."
            ),
        }

    def summary_lines(self) -> List[str]:
        """Return a compact human-readable summary as a list of lines."""
        clarity = self.completion_clarity_snapshot()
        lines = [
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "completion_clarity": self.completion_clarity_snapshot(),
