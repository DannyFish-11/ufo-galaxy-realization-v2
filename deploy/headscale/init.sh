...
echo ""
echo "Watches don't use this key: the gateway issues each watch a single-use key"
echo "during pairing. For that, give the gateway an API key:"
echo "  $HS apikeys create --expiration 365d"
echo "  → put it in GALAXY_HEADSCALE_API_KEY (and GALAXY_HEADSCALE_URL = this server)"
echo ""
